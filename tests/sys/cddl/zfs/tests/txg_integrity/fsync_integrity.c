...
#define FSIZE \
	(64 * (CLUSTERSIZE)) // FSIZE may range from NUM_CHUNKS clusters to 8GB
	...
	for (chunk_idx = 0; chunk_idx < NUM_CHUNKS; chunk_idx++) {
		...
		for (i = 0; i < size / sizeof(uint64_t); i++) {
			...
			if (good_data && (actual_buf[i] != desired_buf[i])) {
				...
				fprintf(stderr,
				    "fsync_integrity: miscompare ends at "
				    "chunk %i, chunk offset %x, file offset %x\n",
				    chunk_idx, chunk_offset, file_offset);
				good_data = 1;
			}
...
worker(void *args)
{
	...
	for (perm_idx = 0; perm_idx < NUM_CHUNKS; perm_idx++) {
		...
		if (thread_num > 0) {
			if (-1 ==
			    sem_wait(&chunk_sems[thread_num - 1][chunk_idx])) {
				perror("sem_wait");
				exit(1);
			}
		}
		...
		if (thread_num < NUM_THREADS - 1) {
			if (sem_post(&chunk_sems[thread_num][chunk_idx]) ==
			    -1) {
				perror("sem_post");
				exit(1);
			}
...
main(int argc, char **argv)
{
	...
	for (rep = 0; rep < NUM_REPETITIONS; rep++) {
		...
		for (pat = 0; pat < sizeof(patterns) / sizeof(patterns[0]);
			...
			/*      pattern_t *pat_p = (void *)(uintptr_t)(const
			 * void *)patterns[pat];*/
			int sem_idx;
			int ofs = 0;
			...
			// Return the file pointer to the beginning prior to
			// mmap
			...
			for (i = 0; i < NUM_THREADS - 1; i++) {
				for (sem_idx = 0; sem_idx < NUM_CHUNKS;
				     sem_idx++) {
					if (sem_init(&chunk_sems[i][sem_idx], 0,
						0)) {
						perror("sem_init");
						exit(1);
					}
