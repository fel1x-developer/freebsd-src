...
#define FSIZE \
	(512 * (CLUSTERSIZE)) // FSIZE may range from NUM_CHUNKS clusters to 8GB
#define USE_THREADS 1
#define USE_MMAP 0

typedef struct {
	// partitions describes the boundaries between chunks.  Each element is
	// a fraction of the filesize in 1.31 fixed point format.  So the
	// boundary between chunk n and chunk n-1 is (FSIZE * partitions[n-1] /
	// (1<<31) . partitions[-1] is understood to be 0 and
	// partitions[NUM_CHUNKS] must be 1.0 partitions is sorted, of course.
	// Partition boundaries must be dword aligned.  Thus, in order to work
	// with multiple values of FSIZE, partitions values must be aligned to
	// multiples of 8 / (NUM_CHUNKS * CLUSTERSIZE) = 1 / 524288 = 0x0.00002
 ...
 * end is exclusive */
void
get_chunk_range(const pattern_t *pat, int chunk, uint32_t *begin, uint32_t *end)
{
	if (chunk == 0) {
		*begin = 0;
	} else {
		*begin = (uint32_t)((uint64_t)FSIZE *
			(uint64_t)pat->partitions[chunk - 1] >>
		    31);
	}
	*end = (uint32_t)((uint64_t)FSIZE * (uint64_t)pat->partitions[chunk] >>
	    31);
}

typedef enum { leader_syncs = 0, follower_syncs, NUM_PHASES } phase_t;
...
const pattern_t trivial_pattern = {
	{ 0x2000000, 0x4000000, 0x6000000, 0x8000000, 0xa000000, 0xc000000,
	    0xe000000, 0x10000000, 0x12000000, 0x14000000, 0x16000000,
	    0x18000000, 0x1a000000, 0x1c000000, 0x1e000000, 0x20000000,
	    0x22000000, 0x24000000, 0x26000000, 0x28000000, 0x2a000000,
	    0x2c000000, 0x2e000000, 0x30000000, 0x32000000, 0x34000000,
	    0x36000000, 0x38000000, 0x3a000000, 0x3c000000, 0x3e000000,
	    0x40000000, 0x42000000, 0x44000000, 0x46000000, 0x48000000,
	    0x4a000000, 0x4c000000, 0x4e000000, 0x50000000, 0x52000000,
	    0x54000000, 0x56000000, 0x58000000, 0x5a000000, 0x5c000000,
	    0x5e000000, 0x60000000, 0x62000000, 0x64000000, 0x66000000,
	    0x68000000, 0x6a000000, 0x6c000000, 0x6e000000, 0x70000000,
	    0x72000000, 0x74000000, 0x76000000, 0x78000000, 0x7a000000,
	    0x7c000000, 0x7e000000, 0x80000000 },
...
const pattern_t pat31 = { {
	...
	0xc8cbea4e3899aca5 };
const pattern_t *patterns[] = { &trivial_pattern, &pat0, &pat1, &pat2, &pat3,
	&pat4, &pat5, &pat6, &pat7, &pat8, &pat9, &pat10, &pat11, &pat12,
	&pat13, &pat14, &pat15, &pat16, &pat17, &pat18, &pat19, &pat20, &pat21,
	&pat22, &pat23, &pat24, &pat25, &pat26, &pat27, &pat28, &pat29, &pat30,
	&pat31 };
...
marker_fill(uint64_t *buf, int file_ofs, size_t len, int is_follower)
{
	...
	for (ofs = file_ofs; ofs < file_ofs + len; ofs += sizeof(uint64_t)) {
		uint64_t mark =
		    ((is_follower ? (uint64_t)follow_mark : (uint64_t)lead_mark)
			<< (uint64_t)32) |
	...
	for (chunk_idx = 0; chunk_idx < NUM_CHUNKS; chunk_idx++) {
		...
		for (i = 0; i < size / sizeof(uint64_t); i++) {
			...
			if (good_data && (actual_buf[i] != desired_buf[i])) {
			    ...
			    (actual_buf[i] == desired_buf[i])) {
				fprintf(stderr,
				    "txg_integrity: miscompare ends at "
				    "chunk %i, chunk offset %x, file offset %x\n",
				    chunk_idx, chunk_offset, file_offset);
				good_data = 1;
			}
...
main(int argc, char **argv)
{
	...
	// follower_chunks bitmap must have enough space to represent every
	// chunk
	assert(NUM_CHUNKS <= 8 * sizeof(((pattern_t *)0)->follower_chunks));

	for (rep = 0; rep < NUM_REPETITIONS; rep++) {
		printf("Starting repetition %d\n", rep);
		for (pat = 0; pat < sizeof(patterns) / sizeof(patterns[0]);
		     pat++) {
			void *pat_p = (void *)(uintptr_t)(const void *)
			    patterns[pat];
			for (phase = leader_syncs; phase < NUM_PHASES;
			     phase++) {
				int sem_idx;
				int ofs = 0;
				...
				// Zero-fill the file to avoid fragmentation, as
				// recommended by mmap(2).
				for (ofs = 0; ofs < FSIZE; ofs += CLUSTERSIZE) {
					char buffer[CLUSTERSIZE];
					bzero(buffer, CLUSTERSIZE);
					if (-1 ==
					    write(outfd, buffer, CLUSTERSIZE)) {
						perror("write");
						exit(1);
					}
				}
...
#if USE_MMAP
				// mmap the file
				p_file = mmap(0, FSIZE, PROT_READ | PROT_WRITE,
				    MAP_SHARED, outfd, 0);
				...
				for (sem_idx = 0; sem_idx < NUM_CHUNKS;
				     sem_idx++) {
					if (sem_init(&chunk_sems[sem_idx], 0,
						0)) {
						perror("sem_init");
						exit(1);
					}
				...
				if (pthread_create(&follower_th, NULL, follower,
					pat_p)) {
					perror("pthread_create follower");
					exit(1);
				}
				if (pthread_create(&leader_th, NULL, leader,
					pat_p)) {
