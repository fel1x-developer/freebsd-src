...
ATF_TC_HEAD(snl_verify_core_parsers, tc)
{
	atf_tc_set_md_var(tc, "descr",
	    "Tests snl(3) core nlmsg parsers are correct");
}
...
{
	atf_tc_set_md_var(tc, "descr",
	    "Tests snl(3) route parsers are correct");
}
...
{
	atf_tc_set_md_var(tc, "descr",
	    "Tests snl(3) correctly parsing capped errors");
}
...
ATF_TC_BODY(snl_parse_errmsg_capped, tc)
{
	...
	ATF_CHECK(rx_hdr->nlmsg_len ==
	    sizeof(struct nlmsghdr) + sizeof(struct nlmsgerr));
...
{
	atf_tc_set_md_var(tc, "descr",
	    "Tests snl(3) correctly parsing capped errors with extack");
}
...
{
	atf_tc_set_md_var(tc, "descr",
	    "Tests snl(3) correctly parsing errors with extack");
}
