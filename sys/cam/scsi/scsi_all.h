...
#else  /* _KERNEL */
int scsi_command_string(struct cam_device *device, struct ccb_scsiio *csio,
    struct sbuf *sb);
int scsi_sense_sbuf(struct cam_device *device, struct ccb_scsiio *csio,
    struct sbuf *sb, scsi_sense_string_flags flags);
char *scsi_sense_string(struct cam_device *device, struct ccb_scsiio *csio,
    char *str, int str_len);
void scsi_sense_print(struct cam_device *device, struct ccb_scsiio *csio,
    FILE *ofile);
#endif /* _KERNEL */

const char *scsi_op_desc(uint16_t opcode, struct scsi_inquiry_data *inq_data);
char *scsi_cdb_string(uint8_t *cdb_ptr, char *cdb_string, size_t len);
...
struct scsi_vpd_id_descriptor *scsi_get_devid(struct scsi_vpd_device_id *id,
    uint32_t len, scsi_devid_checkfn_t ck_fn);
struct scsi_vpd_id_descriptor *
scsi_get_devid_desc(struct scsi_vpd_id_descriptor *desc, uint32_t len,
    scsi_devid_checkfn_t ck_fn);

int scsi_transportid_sbuf(struct sbuf *sb, struct scsi_transportid_header *hdr,
    uint32_t valid_len);
...
void scsi_read_capacity(struct ccb_scsiio *csio, uint32_t retries,
    void (*cbfcnp)(struct cam_periph *, union ccb *), uint8_t tag_action,
    struct scsi_read_capacity_data *, uint8_t sense_len, uint32_t timeout);
...
void scsi_synchronize_cache(struct ccb_scsiio *csio, uint32_t retries,
    void (*cbfcnp)(struct cam_periph *, union ccb *), uint8_t tag_action,
...
int scsi_devid_match(uint8_t *rhs, size_t rhs_len, uint8_t *lhs,
    size_t lhs_len);
