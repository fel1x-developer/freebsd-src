...
#define PAIRS(s1, d1, s2, d2)                  \
	((((s1) == (s2)) && ((d1) == (d2))) || \
	    (((s1) == (d2)) && ((d1) == (s2))))
#define IPPAIR(s1, d1, s2, d2) \
	PAIRS((s1).s_addr, (d1).s_addr, (s2).s_addr, (d2).s_addr)
...
#endif
extern int ipf_tcp_age(struct ipftqent *, struct fr_info *, struct ipftq *, int,
    int);
extern int ipf_tcpinwindow(struct fr_info *, struct tcpdata *, struct tcpdata *,
    tcphdr_t *, int);

extern int ipf_state_add(ipf_main_softc_t *, fr_info_t *, ipstate_t **, u_int);
