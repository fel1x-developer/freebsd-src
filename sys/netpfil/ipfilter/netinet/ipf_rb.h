 ...
 *
 */
typedef enum rbcolour_e { C_BLACK = 0, C_RED = 1 } rbcolour_t;
...
#define RBI_CODE(_n, _t, _f, _cmp)                                             \
	...
	void _n##_rb_walktree(struct _n##_rb_head *, _n##_rb_walker_t,         \
	    void *);                                                           \
                                                                               \
	static void rotate_left(struct _n##_rb_head *head, _t *node)           \
	{                                                                      \
		_t *parent, *tmp1, *tmp2;                                      \
                                                                               \
		...
		node->_f.parent = tmp1;                                        \
                                                                               ...
                                                                               \
	static void rotate_right(struct _n##_rb_head *head, _t *node)          \
	{                                                                      \
		_t *parent, *tmp1, *tmp2;                                      \
                                                                               \
		parent = node->_f.parent;                                      \
		...
		node->_f.parent = tmp1;                                        \
                                                                               ...
                                                                               \
	void _n##_rb_insert(struct _n##_rb_head *head, _t *node)               \
	{                                                                      \
		...
		*p = node;                                                     \
		...
		node->_f.parent = parent;                                      \
                                                                               \
		while (                                                        \
		    (node != &_n##_rb_zero) && (parent->_f.colour == C_RED)) { \
		...
		head->count++;                                                 \
                                                                               ...
                                                                               \
	static void deleteblack(struct _n##_rb_head *head, _t *parent,         \
	    _t *node)                                                          \
	{                                                                      \
		_t *tmp;                                                       \
                                                                               \
		while (                                                        \
		    (node == &_n##_rb_zero || node->_f.colour == C_BLACK) &&   \
		    node != &head->top) {                                      \
		...
		if (node != &_n##_rb_zero)                                     \
			node->_f.colour = C_BLACK;                             \
                                                                               ...
                                                                               \
	_t *_n##_rb_delete(struct _n##_rb_head *head, _t *node)                \
	{                                                                      \
	...
	colour:                                                                \
		...
		return (old);                                                  \
                                                                               ...
                                                                               \
	void _n##_rb_init(struct _n##_rb_head *head)                           \
	{                                                                      \
		...
		_n##_rb_zero._f.parent = &_n##_rb_zero;                        \
                                                                               ...
                                                                               \
	void _n##_rb_walktree(struct _n##_rb_head *head,                       \
		...
		for (;;) {                                                     \
			...
			while ((node->_f.parent->_f.right == node) &&          \
				...
				node = node->_f.parent;                        \
                                                                               ...
                                                                               \
			node = prev;                                           \
			for (node = node->_f.parent->_f.right;                 \
			     node != &_n##_rb_zero; node = node->_f.left)      \
				prev = node;                                   \
			next = prev;                                           \
                                                                               \
			if (node != &_n##_rb_zero)                             \
			...
			if (node == &_n##_rb_zero)                             \
				break;                                         \
                                                                               ...
                                                                               \
	_t *_n##_rb_search(struct _n##_rb_head *head, void *key)               \
	{                                                                      \
		int match;                                                     \
		_t *node;                                                      \
		node = head->top._f.right;                                     \
