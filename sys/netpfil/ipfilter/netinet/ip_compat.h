...
#define NETBSD_GE_REV(x) \
	(defined(__NetBSD_Version__) && (__NetBSD_Version__ >= (x)))
#define NETBSD_GT_REV(x) \
	(defined(__NetBSD_Version__) && (__NetBSD_Version__ > (x)))
#define NETBSD_LT_REV(x) \
	(defined(__NetBSD_Version__) && (__NetBSD_Version__ < (x)))
...
#define MUTEX_INIT(x, y) mtx_init(&(x)->ipf_lk, (y), NULL, MTX_DEF)
...
struct ifnet {
	...
	int (*if_output)(struct ifnet *, struct mbuf *, const struct sockaddr *,
	    struct route *);
};
...
#define COPYIFNAME(v, x, b) \
	(void)strncpy(b, ((struct ifnet *)x)->if_xname, LIFNAMSIZ)
...
#define COPYDATA(m, o, l, b) bcopy(MTOD((mb_t *)m, char *) + (o), (b), (l))
#define COPYBACK(m, o, l, b) bcopy((b), MTOD((mb_t *)m, char *) + (o), (l))
...
#define MUTEX_DESTROY(x) eMmutex_destroy(&(x)->ipf_emu, __FILE__, __LINE__)
#define MUTEX_ENTER(x) eMmutex_enter(&(x)->ipf_emu, __FILE__, __LINE__)
#define MUTEX_EXIT(x) eMmutex_exit(&(x)->ipf_emu, __FILE__, __LINE__)
#define MUTEX_INIT(x, y) eMmutex_init(&(x)->ipf_emu, y, __FILE__, __LINE__)
#define MUTEX_NUKE(x) bzero((x), sizeof(*(x)))

#define MUTEX_DOWNGRADE(x) eMrwlock_downgrade(&(x)->ipf_emu, __FILE__, __LINE__)
#define MUTEX_TRY_UPGRADE(x) \
	eMrwlock_try_upgrade(&(x)->ipf_emu, __FILE__, __LINE__)
#define READ_ENTER(x) eMrwlock_read_enter(&(x)->ipf_emu, __FILE__, __LINE__)
...
#define WRITE_ENTER(x) eMrwlock_write_enter(&(x)->ipf_emu, __FILE__, __LINE__)
	...
	do {                                                      \
		...
		*(fin)->fin_mp = (m);                             \
		if (_o->m_flags & M_PKTHDR) {                     \
			(m)->m_pkthdr.len += _o->m_pkthdr.len;    \
			(m)->m_pkthdr.rcvif = _o->m_pkthdr.rcvif; \
		}                                                 \
	} while (0)
#endif
#ifndef M_DUP
...
#define KMALLOC(a, b) (a) = (b)new_kmem_alloc(sizeof(*(a)), KMEM_NOSLEEP)
...
#define COPYIFNAME(v, x, b) ipf_getifname((struct ifnet *)x, b)
#endif
...
#define TCPF_ALL \
	(TH_FIN | TH_SYN | TH_RST | TH_PUSH | TH_ACK | TH_URG | TH_ECN | TH_CWR)
