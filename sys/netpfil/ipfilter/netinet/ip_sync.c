	...
	while (uio->uio_resid > 0) {
		...
		if (uio->uio_resid >= sizeof(sh)) {
			...
			if (softs->ipf_sync_debug > 8)
				printf(
				    "[%d] Read v:%d p:%d cmd:%d table:%d rev:%d len:%d magic:%x\n",
		...
		if (uio->uio_resid >= sh.sm_len) {
			...
			if (softs->ipf_sync_debug > 2)
				printf("uiomove(data) %s %d bytes, got %d\n",
				    "insufficient data, need", sh.sm_len,
				    (int)uio->uio_resid);
			IPFERROR(110007);
			return (EAGAIN);
		}
	...
	case SMC_UPDATE:
		...
		if (softs->ipf_sync_debug > 4)
			printf("[%d] Update age %lu state %d/%d \n", sp->sm_num,
			    su.stu_age, su.stu_state[0], su.stu_state[1]);
		...
		if (softs->ipf_sync_debug > 6)
			printf(
			    "[%d] Data from state v:%d p:%d cmd:%d table:%d rev:%d\n",
...
ipf_sync_new(ipf_main_softc_t *softc, int tab, fr_info_t *fin, void *ptr)
{
	...
	bcopy((char *)&sl->sl_hdr, (char *)&sle->sle_hdr, sizeof(sle->sle_hdr));
...
/* ------------------------------------------------------------------------ */
int
ipf_sync_ioctl(ipf_main_softc_t *softc, caddr_t data, ioctlcmd_t cmd, int mode,
    int uid, void *ctx)
{
	ipf_sync_softc_t *softs = softc->ipf_sync_soft;
	int error, i;
