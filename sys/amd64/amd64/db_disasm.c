...
static void db_print_address(const char *seg, int size, int rex,
    struct i_addr *addrp);
static db_addr_t db_read_address(db_addr_t loc, int short_addr, int rex,
...
db_disasm(db_addr_t loc, bool altfmt)
{
	...
	for (first = TRUE; i_mode != 0; i_mode >>= 8, first = FALSE) {
		...
		case El:
			db_print_address(seg, (rex & REX_W) ? QUAD : LONG, rex,
			    &address);
			break;
