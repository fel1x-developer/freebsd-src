 ...
 * word in map
 */
#define BT_WIM(bitmap, bitindex) ((bitmap)[(bitindex) >> BT_ULSHIFT])
/*
 * bit in word
 */
#define BT_BIW(bitindex) (1UL << ((bitindex) & BT_ULMASK))

#ifdef _LP64
#define BT_WIM32(bitmap, bitindex) ((bitmap)[(bitindex) >> BT_ULSHIFT32])

#define BT_BIW32(bitindex) (1UL << ((bitindex) & BT_ULMASK32))
#endif
 ...
 */
#define BT_BITOUL(nbits) (((nbits) + BT_NBIPUL - 1l) / BT_NBIPUL)
#define BT_SIZEOFMAP(nbits) (BT_BITOUL(nbits) * sizeof(ulong_t))
...
#ifdef _LP64
#define BT_BITOUL32(nbits) (((nbits) + BT_NBIPUL32 - 1l) / BT_NBIPUL32)
#define BT_SIZEOFMAP32(nbits) (BT_BITOUL32(nbits) * sizeof(uint_t))
 ...
 */
#define BIT_ONLYONESET(u) ((((u) == 0) ? 0 : ((u) & ((u)-1)) == 0))
...
#define BT_ATOMIC_SET(bitmap, bitindex)                      \
	{                                                    \
		atomic_or_ulong(&(BT_WIM(bitmap, bitindex)), \
		    BT_BIW(bitindex));                       \
	}
#define BT_ATOMIC_CLEAR(bitmap, bitindex)                     \
	{                                                     \
		atomic_and_ulong(&(BT_WIM(bitmap, bitindex)), \
		    ~BT_BIW(bitindex));                       \
	}
