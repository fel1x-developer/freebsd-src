...
static const uint32_t SHA256_K[64] = { 0x428a2f98, 0x71374491, 0xb5c0fbcf,
...
static const uint64_t SHA512_K[80] = { 0x428A2F98D728AE22ULL,
...
SHA256Transform(uint32_t *H, const uint8_t *cp)
{
	...
	for (t = 16; t < 64; t++)
		W[t] = SIGMA1_256(W[t - 2]) + W[t - 7] + SIGMA0_256(W[t - 15]) +
		    W[t - 16];
...
SHA512Transform(uint64_t *H, const uint8_t *cp)
{
	...
	for (t = 16; t < 80; t++)
		W[t] = SIGMA1_512(W[t - 2]) + W[t - 7] + SIGMA0_512(W[t - 15]) +
		    W[t - 16];
...
SHA256(uint32_t *H, const void *buf, uint64_t size, zio_cksum_t *zcp)
{
	...
	ZIO_SET_CHECKSUM(zcp, (uint64_t)H[0] << 32 | H[1],
	    (uint64_t)H[2] << 32 | H[3], (uint64_t)H[4] << 32 | H[5],
	    (uint64_t)H[6] << 32 | H[7]);
}
    ...
    const void *ctx_template __unused, zio_cksum_t *zcp)
{
	...
	uint64_t H[] = { 0x22312194FC2BF72CULL, 0x9F555FA3C84C64C2ULL,
