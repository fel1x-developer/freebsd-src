...
#define INKERNEL(x)                                                        \
	(((x) <= VM_MAX_KERNEL_ADDRESS && (x) >= VM_MIN_KERNEL_ADDRESS) || \
	...
	while (pc != 0) {
		...
		*fpstack++ = sp;
...
#endif /* XXX */
		{
			if (SV_PROC_FLAG(p, SV_ILP32)) {
				pc = dtrace_fuword32(
				    (void *)(sp + RETURN_OFFSET));
				sp = dtrace_fuword32((void *)sp);
			} else {
				pc = dtrace_fuword64(
				    (void *)(sp + RETURN_OFFSET64));
				sp = dtrace_fuword64((void *)sp);
			}
		}
...
dtrace_getarg(int arg, int aframes)
{
	...
	for (i = 1; i <= aframes; i++) {
		...
		if ((long)(fp[1]) == (long)trapexit) {
			 ...
			 * the regs structure that was pushed when we took the
			 * trap.  To get this structure, we must increment
			 * beyond the frame structure.  If the argument that
			 * we're seeking is passed on the stack, we'll pull the
			 * true stack pointer out of the saved registers and
			 * decrement our argument by the number of arguments
			 * passed in registers; if the argument we're seeking is
			 * passed in regsiters, we can just load it directly.
			 */
#ifdef __powerpc64__
    ...
    volatile uint16_t *flags)
{
	...
	if (dtrace_copycheck(uaddr, kaddr, size)) {
		error = copyinstr((const void *)uaddr, (void *)kaddr, size,
		    &actual);
