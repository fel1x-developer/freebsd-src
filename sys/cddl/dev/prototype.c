...
static void prototype_getargdesc(void *, dtrace_id_t, void *,
    dtrace_argdesc_t *);
...
static dtrace_pops_t prototype_pops = { .dtps_provide = prototype_provide,
...
prototype_load(void *dummy)
{
	 ...
	 */
	prototype_cdev = make_dev(&prototype_cdevsw, 0, UID_ROOT, GID_WHEEL,
	    0600, "dtrace/prototype");
...
static int
prototype_open(struct cdev *dev __unused, int oflags __unused,
    int devtype __unused, struct thread *td __unused)
{
	return (0);
}

SYSINIT(prototype_load, SI_SUB_DTRACE_PROVIDER, SI_ORDER_ANY, prototype_load,
    NULL);
SYSUNINIT(prototype_unload, SI_SUB_DTRACE_PROVIDER, SI_ORDER_ANY,
    prototype_unload, NULL);
