...
#endif
	size_t r;
	u64b_t kw[WCNT + 4 + RCNT * 2]; /* key schedule words : chaining vars +
					   tweak + "rotation"*/
#else
...
#define I256(R)                                                    \
	...
	Skein_Show_R_Ptr(BLK_BITS, &ctx->h, SKEIN_RND_KEY_INJECT, Xptr);
		...
		for (r = 1; r < 2 * RCNT;
		     r += 2 * SKEIN_UNROLL_256) /* loop thru it */
#endif
		{
...
#error "Invalid SKEIN_UNROLL_512"               /* sanity check on unroll count */
	...
	u64b_t kw[WCNT + 4]; /* key schedule words : chaining vars + tweak */
#endif
	u64b_t X0, X1, X2, X3, X4, X5, X6,
	    X7;		/* local copy of vars, for speed */
	u64b_t w[WCNT]; /* local copy of input block */
#ifdef SKEIN_DEBUG
	const u64b_t *
	...
	do {
		...
		ks[6] = ctx->X[6];
		ks[7] = ctx->X[7];
		ks[8] = ks[0] ^ ks[1] ^ ks[2] ^ ks[3] ^ ks[4] ^ ks[5] ^ ks[6] ^
		    ks[7] ^ SKEIN_KS_PARITY;
...
#define I512(R)                                                    \
	...
	Skein_Show_R_Ptr(BLK_BITS, &ctx->h, SKEIN_RND_KEY_INJECT, Xptr);
		...
		for (r = 1; r < 2 * RCNT;
		     r += 2 * SKEIN_UNROLL_512) /* loop thru it */
...
#error "Invalid SKEIN_UNROLL_1024"              /* sanity check on unroll count */
	...
	u64b_t w[WCNT]; /* local copy of input block */
#ifdef SKEIN_DEBUG
	const u64b_t *Xptr[16]; /* use for debugging (help compiler put Xn in
				   registers) */
	Xptr[0] = &X00;
	Xptr[1] = &X01;
	Xptr[2] = &X02;
	...
	do {
		...
		ks[14] = ctx->X[14];
		ks[15] = ctx->X[15];
		ks[16] = ks[0] ^ ks[1] ^ ks[2] ^ ks[3] ^ ks[4] ^ ks[5] ^ ks[6] ^
		    ks[7] ^ ks[8] ^ ks[9] ^ ks[10] ^ ks[11] ^ ks[12] ^ ks[13] ^
		    ks[14] ^ ks[15] ^ SKEIN_KS_PARITY;
...
#define R1024(p0, p1, p2, p3, p4, p5, p6, p7, p8, p9, pA, pB, pC, pD, pE, pF, \
	...
	Round1024(p0, p1, p2, p3, p4, p5, p6, p7, p8, p9, pA, pB, pC, pD, pE, \
	    pF, ROT, rn) Skein_Show_R_Ptr(BLK_BITS, &ctx->h, rn, Xptr);
...
#define I1024(R)                                                    \
	...
	Skein_Show_R_Ptr(BLK_BITS, &ctx->h, SKEIN_RND_KEY_INJECT, Xptr);
		...
		for (r = 1; r <= 2 * RCNT;
		     r += 2 * SKEIN_UNROLL_1024) /* loop thru it */
#endif
		{
