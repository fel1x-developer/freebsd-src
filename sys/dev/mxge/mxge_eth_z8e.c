...
mxge_eth_z8e_fw_modevent(module_t mod, int type, void *unused)
{
	...
	case MOD_LOAD:
		...
		fp = firmware_register("mxge_eth_z8e", eth_z8e,
		    (size_t)eth_z8e_length, eth_z8e_uncompressed_length, NULL);
		if (fp == NULL)
			goto fail_0;
		return (0);
	fail_0:
...
static moduledata_t mxge_eth_z8e_fw_mod = { "mxge_eth_z8e_fw",
	mxge_eth_z8e_fw_modevent, 0 };
DECLARE_MODULE(mxge_eth_z8e_fw, mxge_eth_z8e_fw_mod, SI_SUB_DRIVERS,
    SI_ORDER_FIRST);
