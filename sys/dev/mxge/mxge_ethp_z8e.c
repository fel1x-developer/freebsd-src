...
mxge_ethp_z8e_fw_modevent(module_t mod, int type, void *unused)
{
	...
	case MOD_LOAD:
		...
		fp = firmware_register("mxge_ethp_z8e", ethp_z8e,
		    (size_t)ethp_z8e_length, ethp_z8e_uncompressed_length,
		    NULL);
		if (fp == NULL)
			goto fail_0;
		return (0);
...
static moduledata_t mxge_ethp_z8e_fw_mod = { "mxge_ethp_z8e_fw",
	mxge_ethp_z8e_fw_modevent, 0 };
DECLARE_MODULE(mxge_ethp_z8e_fw, mxge_ethp_z8e_fw_mod, SI_SUB_DRIVERS,
    SI_ORDER_FIRST);
