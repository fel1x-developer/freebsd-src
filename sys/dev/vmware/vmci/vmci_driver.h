...
#define VMCI_LOG_DEBUG(_args, ...) log(LOG_DEBUG, _args, ##__VA_ARGS__)
#endif /* !VMCI_DEBUG_LOGGING */
#define VMCI_LOG_INFO(_args, ...) log(LOG_INFO, _args, ##__VA_ARGS__)
#define VMCI_LOG_WARNING(_args, ...) log(LOG_WARNING, _args, ##__VA_ARGS__)
#define VMCI_LOG_ERROR(_args, ...) log(LOG_ERR, _args, ##__VA_ARGS__)
...
void vmci_read_datagrams_from_port(vmci_io_handle io_handle,
    vmci_io_port dg_in_port, uint8_t *dg_in_buffer, size_t dg_in_buffer_size);
