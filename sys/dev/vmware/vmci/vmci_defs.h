...
#define VMCI_ANON_SRC_HANDLE \
	VMCI_MAKE_HANDLE(VMCI_ANON_SRC_CONTEXT_ID, VMCI_ANON_SRC_RESOURCE_ID)
...
#define VMCI_EVENT_CTX_ID_UPDATE \
	0			 // Only applicable to guest
				 // endpoints
#define VMCI_EVENT_CTX_REMOVED 1 // Applicable to guest and host
#define VMCI_EVENT_QP_RESUMED \
	2 // Only applicable to guest
	  // endpoints
#define VMCI_EVENT_QP_PEER_ATTACH \
	3 // Applicable to guest, host
	  // and VMX
#define VMCI_EVENT_QP_PEER_DETACH \
	4 // Applicable to guest, host
	  // and VMX
#define VMCI_EVENT_MEM_ACCESS_ON \
	5 // Applicable to VMX and vmk. On
	  // vmk, this event has the
	  // Context payload type
#define VMCI_EVENT_MEM_ACCESS_OFF \
	6 // Applicable to VMX and vmk.
	  // Same as above for the payload
	  // type
#define VMCI_EVENT_GUEST_PAUSED \
	7 // Applicable to vmk. This
	  // event has the Context
	  // payload type
#define VMCI_EVENT_GUEST_UNPAUSED \
	8 // Applicable to vmk. Same as
...
#define VMCI_EVENT_VALID(_event)                                          \
	(_event < VMCI_EVENT_MAX && _event != VMCI_EVENT_MEM_ACCESS_ON && \
	    _event != VMCI_EVENT_MEM_ACCESS_OFF &&                        \
    ...
    uint64_t *consumer_head)
{
	...
	if (producer_tail)
		*producer_tail = vmci_queue_header_producer_tail(
		    produce_q_header);

	if (consumer_head)
		*consumer_head = vmci_queue_header_consumer_head(
		    consume_q_header);
}
