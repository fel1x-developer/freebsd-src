...
static CpaStatus
qat_ocf_session_update(struct qat_ocf_session *ocf_session, Cpa8U *newCipher,
    Cpa8U *newAuth, Cpa32U newAADLength)
{
	...
	if (newAADLength != QAT_OCF_AAD_NOCHANGE) {
		...
		status = LacAlgChain_SessionAADUpdate(pSessionDesc,
		    newAADLength);
		if (CPA_STATUS_SUCCESS != status)
			return status;
	}
	...
	if (newCipher) {
		status = LacAlgChain_SessionCipherKeyUpdate(pSessionDesc,
		    newCipher);
		if (CPA_STATUS_SUCCESS != status)
			return status;
	}
	...
	if (newAuth) {
		status = LacAlgChain_SessionAuthKeyUpdate(pSessionDesc,
		    newAuth);
		if (CPA_STATUS_SUCCESS != status)
			return status;
	}
    ...
    struct cryptop *crp)
{
	...
	if (is_sep_aad_supported(csp)) {
		...
		if ((ocf_dsession->encSession.sessionCtx &&
			ocf_dsession->encSession.aadLen !=
			    crp->crp_aad_length) ||
		    (ocf_dsession->decSession.sessionCtx &&
			ocf_dsession->decSession.aadLen !=
			    crp->crp_aad_length)) {
	...
	if (crp->crp_cipher_key || crp->crp_auth_key ||
		...
		status = qat_ocf_session_update(&ocf_dsession->encSession,
		    cipherKey, authKey, newAADLength);
		if (CPA_STATUS_SUCCESS != status)
			return status;
