	...
	if (CPA_CY_SYM_OP_HASH != pSessionDesc->symOperation) {
		...
		case CPA_CY_SYM_CIPHER_AES_CCM: {
			if (CPA_STATUS_SUCCESS !=
				...
				pRequest->pAdditionalAuthData, pRequest->pIv,
    ...
    icp_qat_fw_la_bulk_req_t *pCurrentQatMsg)
{
	...
	memset((pMsgDummy +
		   (LAC_LONG_WORD_IN_BYTES * LAC_SIZE_OF_CACHE_HDR_IN_LW)),
	    0, (LAC_LONG_WORD_IN_BYTES * LAC_SIZE_OF_CACHE_TO_CLEAR_IN_LW));
	memcpy(pMsgDummy +
		(LAC_LONG_WORD_IN_BYTES * LAC_START_OF_CACHE_FTR_IN_LW),
	    pCacheDummyFtr,
	    (LAC_LONG_WORD_IN_BYTES * LAC_SIZE_OF_CACHE_FTR_IN_LW));

	SalQatMsg_CmnMidWrite(pCurrentQatMsg, pRequest,
	    (CPA_DP_BUFLIST == pRequest->srcBufferLen ? QAT_COMN_PTR_TYPE_SGL :
							QAT_COMN_PTR_TYPE_FLAT),
	    pRequest->srcBuffer, pRequest->dstBuffer, pRequest->srcBufferLen,
	    pRequest->dstBufferLen);

	/* Write Request Params */
	if (pSessionDesc->isCipher) {

		LacSymQat_CipherRequestParamsPopulate(pSessionDesc,
		    pCurrentQatMsg, pRequest->cryptoStartSrcOffsetInBytes,
		    pRequest->messageLenToCipherInBytes, pRequest->iv,
		    pRequest->pIv);
	}
	...
	if (pSessionDesc->isAuth) {
		 ...
		 */
		unsigned long cplen = (unsigned long)&(
		...
		if (CPA_TRUE == pSessionDesc->isAuthEncryptOp) {
			pAuthReqPars->hash_state_sz = LAC_BYTES_TO_QUADWORDS(
			    pAuthReqPars->u2.aad_sz);
		} else if (CPA_CY_SYM_HASH_SNOW3G_UIA2 ==
			pSessionDesc->hashAlgorithm ||
		    CPA_CY_SYM_HASH_ZUC_EIA3 == pSessionDesc->hashAlgorithm) {
			pAuthReqPars->hash_state_sz = LAC_BYTES_TO_QUADWORDS(
    ...
    Cpa32U *pSessionCtxSizeInBytes)
{
	...
	status = cpaCySymSessionCtxGetSize(instanceHandle, pSessionSetupData,
    ...
    CpaCySymDpSessionCtx sessionCtx)
{
	...
	status = LacSym_InitSession(instanceHandle, NULL, /* Callback */
	    pSessionSetupData, CPA_TRUE,		  /* isDPSession */
	    sessionCtx);
	return status;
}
...
cpaCySymDpEnqueueOpBatch(const Cpa32U numberRequests,
    CpaCySymDpOpData *pRequests[], const CpaBoolean performOpNow)
{
	icp_qat_fw_la_bulk_req_t *pCurrentQatMsg = NULL;
	icp_comms_trans_handle trans_handle = NULL;
	lac_session_desc_t *pSessionDesc = NULL;
