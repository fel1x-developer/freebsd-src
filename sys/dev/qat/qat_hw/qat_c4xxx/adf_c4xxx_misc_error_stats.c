...
static int
qat_misc_error_show(SYSCTL_HANDLER_ARGS)
{
	struct sbuf sb;
	...
	sbuf_printf(&sb, "\n");
	sbuf_printf(&sb, LINE);
	sbuf_printf(&sb, "| Miscellaneous Error:   %40llu |\n",
...
adf_misc_error_add_c4xxx(struct adf_accel_dev *accel_dev)
{
	...
	misc_er_file = SYSCTL_ADD_PROC(qat_sysctl_ctx,
	    SYSCTL_CHILDREN(qat_sysctl_tree), OID_AUTO, MISC_ERROR_DBG_FILE,
	    CTLTYPE_STRING | CTLFLAG_RD, accel_dev, 0, qat_misc_error_show, "A",
