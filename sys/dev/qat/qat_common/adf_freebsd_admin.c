...
static u8 const_tab[1024] __aligned(1024) = { ADF_CONST_TABLE_VERSION, 0x00,
	...
	0x21, 0x79, 0x14, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00,
	0x00, 0x40, 0x00, 0x00, 0x00, 0x00, 0x16, 0x00, 0x00, 0x00, 0x00, 0x00,
	0x00, 0x00, 0x00, 0x00, 0x00, 0x80, 0x00, 0x00, 0x00, 0x00, 0x12, 0x00,
	...
	,
...
int
adf_put_admin_msg_sync(struct adf_accel_dev *accel_dev, u32 ae, void *in,
    void *out)
{
	...
	int mb_offset = ae * ADF_MAILBOX_STRIDE +
	    admin_csrs_info.mailbox_offset;
	...
	if (received)
		memcpy(out, admin->virt_addr + offset + ADF_ADMINMSG_LEN,
		    ADF_ADMINMSG_LEN);
	else
...
int
adf_get_fw_pke_stats(struct adf_accel_dev *accel_dev, u64 *suc_count,
    u64 *unsuc_count)
{
...
adf_send_admin_init(struct adf_accel_dev *accel_dev)
{
	...
	if (GET_HW_DATA(accel_dev)->query_storage_cap) {
		...
		ret = GET_HW_DATA(accel_dev)->get_storage_enabled(accel_dev,
		    &storage_enabled);
		if (ret) {
			device_printf(GET_DEV(accel_dev),
...
adf_init_admin_comms(struct adf_accel_dev *accel_dev)
{
	...
	admin = kzalloc_node(sizeof(*accel_dev->admin), M_WAITOK | M_ZERO,
	    dev_to_node(GET_DEV(accel_dev)));
	hw_data = accel_dev->hw_device;
	pmisc = &GET_BARS(accel_dev)[hw_data->get_misc_bar_id(hw_data)];
	csr = pmisc->virt_addr;
	ret = bus_dma_mem_create(&admin->dma_mem, accel_dev->dma_tag,
	    FREEBSD_ALLIGNMENT_SIZE, BUS_SPACE_MAXADDR, PAGE_SIZE, 0);
	if (ret != 0) {
		device_printf(GET_DEV(accel_dev),
	...
	ret = bus_dmamap_load(accel_dev->dma_tag, admin->const_tbl_map,
	    (void *)const_tab, 1024, dma_callback, &admin->const_tbl_addr,
	    BUS_DMA_NOWAIT);
	...
	if (accel_dev->aram_info) {
		ret = bus_dmamap_create(accel_dev->dma_tag, 0,
		    &admin->aram_map);
		if (ret != 0) {
			device_printf(GET_DEV(accel_dev),
		...
		ret = bus_dmamap_load(accel_dev->dma_tag, admin->aram_map,
		    (void *)accel_dev->aram_info, sizeof(*accel_dev->aram_info),
		    dma_callback, &admin->aram_map_phys_addr, BUS_DMA_NOWAIT);
	...
	ret = bus_dma_mem_create(&admin->dma_hb, accel_dev->dma_tag,
	    FREEBSD_ALLIGNMENT_SIZE, BUS_SPACE_MAXADDR, PAGE_SIZE, 0);
	if (ret != 0) {
		device_printf(GET_DEV(accel_dev),
		    "Failed to allocate dma buff\n");
