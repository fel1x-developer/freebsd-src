...
static int
adf_sysctl_read_fw_versions(SYSCTL_HANDLER_ARGS)
{
	...
	return SYSCTL_OUT(req, fw_version,
	    strnlen(fw_version, ADF_CFG_MAX_VAL_LEN_IN_BYTES));
}

static int
adf_sysctl_read_hw_versions(SYSCTL_HANDLER_ARGS)
{
	struct adf_accel_dev *accel_dev = arg1;
	...
	return SYSCTL_OUT(req, hw_version,
	    strnlen(hw_version, ADF_CFG_MAX_VAL_LEN_IN_BYTES));
}

static int
adf_sysctl_read_mmp_versions(SYSCTL_HANDLER_ARGS)
{
	struct adf_accel_dev *accel_dev = arg1;
	...
	return SYSCTL_OUT(req, mmp_version,
...
adf_ver_dbg_add(struct adf_accel_dev *accel_dev)
{
	...
	rc = SYSCTL_ADD_OID(qat_sysctl_ctx, SYSCTL_CHILDREN(qat_sysctl_tree),
	    OID_AUTO, "fw_version", CTLTYPE_STRING | CTLFLAG_RD, accel_dev, 0,
	    adf_sysctl_read_fw_versions, "A", "QAT FW version");
	if (!rc)
		goto err;

	rc = SYSCTL_ADD_OID(qat_sysctl_ctx, SYSCTL_CHILDREN(qat_sysctl_tree),
	    OID_AUTO, "hw_version", CTLTYPE_STRING | CTLFLAG_RD, accel_dev, 0,
	    adf_sysctl_read_hw_versions, "A", "QAT HW version");
	if (!rc)
		goto err;

	rc = SYSCTL_ADD_OID(qat_sysctl_ctx, SYSCTL_CHILDREN(qat_sysctl_tree),
	    OID_AUTO, "mmp_version", CTLTYPE_STRING | CTLFLAG_RD, accel_dev, 0,
	    adf_sysctl_read_mmp_versions, "A", "QAT MMP version");
	if (!rc)
		goto err;
