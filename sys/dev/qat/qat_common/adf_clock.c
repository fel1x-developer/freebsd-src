...
adf_clock_debugfs_add(struct adf_accel_dev *accel_dev)
{
	...
	rc = SYSCTL_ADD_UINT(qat_sysctl_ctx, SYSCTL_CHILDREN(qat_sysctl_tree),
	    OID_AUTO, CLK_DBGFS_FILE, CTLFLAG_RD, &hw_data->clock_frequency, 0,
	    "clock frequency");
	HB_SYSCTL_ERR(rc);
	return 0;
}
 ...
 */
int
adf_dev_measure_clock(struct adf_accel_dev *accel_dev, u32 *frequency, u32 min,
    u32 max)
{
	int ret;
	u32 freq;
