    ...
    unsigned int ae, unsigned int image_num)
{
	...
	if (encap_image->img_ptr) {
		...
		ae_data->shareable_ustore = ICP_QAT_SHARED_USTORE_MODE(
		    encap_image->img_ptr->ae_mode);
	...
	ae_slice->region = malloc(sizeof(*ae_slice->region), M_QAT,
	    M_WAITOK | M_ZERO);
	ae_slice->page = malloc(sizeof(*ae_slice->page), M_QAT,
	    M_WAITOK | M_ZERO);
	page = ae_slice->page;
...
qat_uclo_wr_sram_by_words(struct icp_qat_fw_loader_handle *handle,
    unsigned int addr, const unsigned int *val, unsigned int num_in_bytes)
{
	unsigned int outval;
...
qat_uclo_wr_umem_by_words(struct icp_qat_fw_loader_handle *handle,
    unsigned char ae, unsigned int addr, unsigned int *val,
...
qat_uclo_batch_wr_umem(struct icp_qat_fw_loader_handle *handle,
    unsigned char ae, struct icp_qat_uof_batch_init *umem_init_header)
{
...
qat_uclo_fetch_initmem_ae(struct icp_qat_fw_loader_handle *handle,
    struct icp_qat_uof_initmem *init_mem, unsigned int size_range,
    unsigned int *ae)
{
...
qat_uclo_create_batch_init_list(struct icp_qat_fw_loader_handle *handle,
    struct icp_qat_uof_initmem *init_mem, unsigned int ae,
    struct icp_qat_uof_batch_init **init_tab_base)
{
	...
	mem_val_attr = (struct icp_qat_uof_memvar_attr *)((uintptr_t)init_mem +
	    sizeof(struct icp_qat_uof_initmem));

	init_header = *init_tab_base;
	if (!init_header) {
		init_header = malloc(sizeof(*init_header), M_QAT,
		    M_WAITOK | M_ZERO);
		init_header->size = 1;
		...
		if (obj_handle->ae_data[ae].shareable_ustore && !gen4) {
			...
			if (init[ae] == 0 && page->beg_addr_p != 0) {
				qat_hal_wr_coalesce_uwords(handle,
				    (unsigned char)ae, 0, page->beg_addr_p,
				    &fill_data[0]);
			}
			qat_hal_wr_coalesce_uwords(handle, (unsigned char)ae,
			    patt_pos, ustore_size - patt_pos,
...
qat_uclo_init_memory(struct icp_qat_fw_loader_handle *handle)
{
	...
	for (i = 0; i < obj_handle->init_mem_tab.entry_num; i++) {
		...
		initmem = (struct icp_qat_uof_initmem
...
static void *
qat_uclo_find_chunk(struct icp_qat_uof_objhdr *obj_hdr, char *chunk_id,
    void *cur)
{
	int i;
    ...
    char *chunk_id)
{
	...
	file_chunk = (struct icp_qat_uof_filechunkhdr *)(buf +
	...
	for (i = 0; i < file_hdr->num_chunks; i++) {
		if (!strncmp(file_chunk->chunk_id, chunk_id,
			...
			if (file_chunk->checksum !=
				...
				break;
			obj_hdr = malloc(sizeof(*obj_hdr), M_QAT,
			    M_WAITOK | M_ZERO);
    ...
    struct icp_qat_uof_image *image)
{
	...
	code_page = (struct icp_qat_uof_code_page *)((char *)image +
	...
	imp_expr_tab = (struct icp_qat_uof_objtable *)(encap_uof_obj->beg_uof +
	...
	neigh_reg_tab = (struct icp_qat_uof_objtable *)(encap_uof_obj->beg_uof +
    ...
    struct icp_qat_uof_image *img, struct icp_qat_uclo_encap_page *page)
{
	...
	code_page = (struct icp_qat_uof_code_page *)((char *)img +
	    sizeof(struct icp_qat_uof_image));
...
qat_uclo_map_uimage(struct icp_qat_uclo_objhandle *obj_handle,
    struct icp_qat_uclo_encapme *ae_uimage, int max_image)
{
	int i, j;
	...
	for (j = 0; j < max_image; j++) {
		...
		ae_uimage[j].ae_reg =
		    (struct icp_qat_uof_ae_reg *)(((char *)ae_regtab) +
			sizeof(struct icp_qat_uof_objtable));
		init_reg_sym_tab =
...
qat_uclo_map_str_table(struct icp_qat_uclo_objhdr *obj_hdr, char *tab_name,
    struct icp_qat_uof_strtable *str_table)
{
	struct icp_qat_uof_chunkhdr *chunk_hdr;

	chunk_hdr = qat_uclo_find_chunk(
	    (struct icp_qat_uof_objhdr *)obj_hdr->file_buff, tab_name, NULL);
	if (chunk_hdr) {
		int hdr_size;
...
qat_uclo_check_uof_compat(struct icp_qat_uclo_objhandle *obj_handle)
{
	...
	if (!(prod_type & obj_handle->encap_uof_obj.obj_hdr->ac_dev_type)) {
		pr_err("QAT: UOF type 0x%x doesn't match with platform 0x%x\n",
		    obj_handle->encap_uof_obj.obj_hdr->ac_dev_type, prod_type);
		return EINVAL;
	}
...
static int
qat_uclo_init_reg(struct icp_qat_fw_loader_handle *handle, unsigned char ae,
...
static int
qat_uclo_init_reg_sym(struct icp_qat_fw_loader_handle *handle, unsigned int ae,
	...
	for (i = 0; i < encap_ae->init_regsym_num; i++) {
		...
		case ICP_QAT_UOF_INIT_REG:
			qat_uclo_init_reg(handle, ae, ctx_mask,
			    ...
			    (unsigned short)init_regsym->reg_addr, exp_res);
			break;
...
qat_hal_set_modes(struct icp_qat_fw_loader_handle *handle,
    struct icp_qat_uclo_objhandle *obj_handle, unsigned char ae,
    struct icp_qat_uof_image *uof_image)
{
...
qat_uclo_parse_uof_obj(struct icp_qat_fw_loader_handle *handle)
{
	...
	obj_handle->uword_buf = malloc(UWORD_CPYBUF_SIZE * sizeof(uint64_t),
	    M_QAT, M_WAITOK | M_ZERO);
	obj_handle->ustore_phy_size = (obj_handle->prod_type ==
					  ICP_QAT_AC_C4XXX_DEV_TYPE) ?
	    0x2000 :
	    0x4000;
	...
	obj_handle->uimage_num = qat_uclo_map_uimage(obj_handle,
	    obj_handle->ae_uimage, ICP_QAT_UCLO_MAX_AE * ICP_QAT_UCLO_MAX_CTX);
...
qat_uclo_map_suof_file_hdr(const struct icp_qat_fw_loader_handle *handle,
    const struct icp_qat_suof_filehdr *suof_ptr, int suof_size)
{
	...
	min_ver_offset = suof_size -
	    offsetof(struct icp_qat_suof_filehdr, min_ver);
    ...
    struct icp_qat_suof_chunk_hdr *suof_chunk_hdr)
{
	...
	suof_img_hdr->simg_buf = (suof_handle->suof_buf +
	    suof_chunk_hdr->offset + sizeof(*suof_objhdr));
    ...
    struct icp_qat_suof_chunk_hdr *suof_chunk_hdr)
{
	...
	*sym_str = (char *)(uintptr_t)(suof_handle->suof_buf +
	    suof_chunk_hdr->offset + sizeof(str_table_obj->tab_length));
}
	...
	if (!IS_QAT_GEN4(pci_get_device(GET_DEV(handle->accel_dev)))) {
		...
		for (i = 0; i < suof_handle->img_table.num_simgs; i++) {
			if ((suof_img_hdr[i].ae_mask &
				(0x1 << (handle->hal_handle->ae_max_num -
				     1))) != 0) {
				aeMax_img = i;
				break;
			}
    ...
    struct icp_qat_fw_auth_desc *desc)
{
	...
	if (fcuAeBroadcastMask) {
		retry = 0;
		SET_FCU_CSR(handle, FCU_ME_BROADCAST_MASK_TYPE,
		    fcuAeBroadcastMask);
		...
		if (retry > FW_BROADCAST_MAX_RETRY) {
			pr_err(
			    ...
			    fcuSts & FCU_AUTH_STS_MASK, retry);
			return EINVAL;
		}
	}
	return 0;
...
qat_uclo_simg_alloc(struct icp_qat_fw_loader_handle *handle,
    struct icp_firml_dram_desc *dram_desc, unsigned int size)
{
	int ret;

	ret = bus_dma_mem_create(&dram_desc->dram_mem,
	    handle->accel_dev->dma_tag, 1, BUS_SPACE_MAXADDR, size, 0);
	if (ret != 0)
		return ret;
...
static int
qat_uclo_map_auth_fw(struct icp_qat_fw_loader_handle *handle, const char *image,
    unsigned int size, struct icp_firml_dram_desc *img_desc,
    struct icp_qat_fw_auth_desc **desc)
{
	const struct icp_qat_css_hdr *css_hdr = (const struct icp_qat_css_hdr *)
	    image;
	...
	if (((struct icp_qat_css_hdr *)(uintptr_t)virt_addr)->fw_type ==
		...
		auth_desc->img_ae_init_data_high = (unsigned int)(bus_addr >>
		    BITS_IN_DWORD);
		auth_desc->img_ae_init_data_low = (unsigned int)bus_addr;
		bus_addr += ICP_QAT_SIMG_AE_INIT_SEQ_LEN;
		auth_desc->img_ae_insts_high = (unsigned int)(bus_addr >>
		    BITS_IN_DWORD);
    ...
    const void *addr_ptr, int mem_size)
{
	...
	if (handle->fw_auth) {
		...
		if (IS_QAT_GEN4(pci_get_device(GET_DEV(handle->accel_dev)))) {
			device_printf(NULL,
			    "QAT: PKE service is not allowed because ");
			device_printf(NULL, "MMP fw will not be loaded for ");
			device_printf(NULL, "device 0x%x",
			    pci_get_device(GET_DEV(handle->accel_dev)));
			return status;
		}
		...
		status = qat_uclo_wr_sram_by_words(handle,
		    handle->hal_sram_offset, addr_ptr, mem_size);
	}
	return status;
}
...
qat_uclo_map_mof_file_hdr(struct icp_qat_fw_loader_handle *handle,
    const struct icp_qat_mof_file_hdr *mof_ptr, u32 mof_size)
{
	...
	min_ver_offset = mof_size -
	    offsetof(struct icp_qat_mof_file_hdr, min_ver);
...
qat_uclo_seek_obj_inside_mof(struct icp_qat_mof_handle *mobj_handle,
    const char *obj_name, const char **obj_ptr, unsigned int *obj_size)
{
	unsigned int i;
...
{
	if ((strncmp((char *)obj_chunkhdr->chunk_id, ICP_QAT_UOF_IMAG,
		ICP_QAT_MOF_OBJ_CHUNKID_LEN)) == 0) {
		mobj_hdr->obj_buf =
		    (const char *)((unsigned long)obj_chunkhdr->offset +
			mobj_handle->uobjs_hdr);
	} else if ((strncmp((char *)(obj_chunkhdr->chunk_id), ICP_QAT_SUOF_IMAG,
		       ICP_QAT_MOF_OBJ_CHUNKID_LEN)) == 0) {
		mobj_hdr->obj_buf =
		    (const char *)((unsigned long)obj_chunkhdr->offset +
...
qat_uclo_map_objs_from_mof(struct icp_qat_mof_handle *mobj_handle)
{
	...
	for (i = 0; i < uobj_chunk_num; i++) {
		ret = qat_uclo_map_obj_from_mof(mobj_handle,
		    &mof_obj_hdr[*valid_chunks], &uobj_chunkhdr[i]);
		if (ret)
			return ret;
		(*valid_chunks)++;
	}
	...
	for (i = 0; i < sobj_chunk_num; i++) {
		ret = qat_uclo_map_obj_from_mof(mobj_handle,
		    &mof_obj_hdr[*valid_chunks], &sobj_chunkhdr[i]);
		if (ret)
			return ret;
		(*valid_chunks)++;
	}
    ...
    struct icp_qat_mof_chunkhdr *mof_chunkhdr)
{
	...
	*sym_size = *(unsigned int *)(uintptr_t)(mof_chunkhdr->offset +
	    mobj_handle->mof_buf);
	*sym_str = (char *)(uintptr_t)(mobj_handle->mof_buf +
	    mof_chunkhdr->offset + sizeof(str_table_obj->tab_len));
}
    ...
    const char *obj_name, const char **obj_ptr, unsigned int *obj_size)
{
	...
	ret = qat_uclo_seek_obj_inside_mof(mobj_handle, obj_name, obj_ptr,
	    obj_size);
	if (ret)
		return ret;
	return 0;
...
int
qat_uclo_map_obj(struct icp_qat_fw_loader_handle *handle, const void *addr_ptr,
    u32 mem_size, const char *obj_name)
{
	const char *obj_addr;
	u32 obj_size;
	int ret;
	...
	BUILD_BUG_ON(
	    ICP_QAT_UCLO_MAX_AE > (sizeof(handle->hal_handle->ae_mask) * 8));
...
qat_uclo_fill_uwords(struct icp_qat_uclo_objhandle *obj_handle,
    struct icp_qat_uclo_encap_page *encap_page, uint64_t *uword,
    unsigned int addr_p, unsigned int raddr, uint64_t fill)
{
	uint64_t uwrd = 0;
	unsigned int i, addr;
...
qat_uclo_wr_uimage_raw_page(struct icp_qat_fw_loader_handle *handle,
    struct icp_qat_uclo_encap_page *encap_page, unsigned int ae)
{
	...
	while (words_num) {
		...
		for (i = 0; i < cpylen; i++)
			qat_uclo_fill_uwords(obj_handle, encap_page,
			    &obj_handle->uword_buf[i], uw_physical_addr + i,
		...
		if (obj_handle->ae_data[ae].shareable_ustore &&
			...
			qat_hal_wr_coalesce_uwords(handle, (unsigned char)ae,
			    uw_physical_addr, cpylen, obj_handle->uword_buf);
		else
			/* copy the buffer to ustore */
    ...
    struct icp_qat_uof_image *image)
{
	...
	for_each_set_bit(ae, &ae_mask, handle->hal_handle->ae_max_num)
	{
		...
		for (ctx = 0; ctx < ICP_QAT_UCLO_MAX_CTX; ctx++)
			aed->ae_slices[s].cur_page[ctx] = (ctx_mask &
							      (1 << ctx)) ?
			    page :
			    NULL;
...
qat_uclo_wr_suof_img(struct icp_qat_fw_loader_handle *handle)
{
	...
	for (i = 0; i < sobj_handle->img_table.num_simgs; i++) {
		if (qat_uclo_map_auth_fw(handle,
			...
			(unsigned int)(simg_hdr[i].simg_len), &img_desc, &desc))
			goto wr_err;
