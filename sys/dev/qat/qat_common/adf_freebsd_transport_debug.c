...
static int
adf_ring_show(SYSCTL_HANDLER_ARGS)
{
	...
	sbuf_new_for_sysctl(&sb, NULL, 128, req);
	{
		...
		sbuf_printf(&sb, "ring num %d, bank num %d\n",
		    ring->ring_number, ring->bank->bank_number);
		sbuf_printf(&sb, "head %x, tail %x, empty: %d\n", head, tail,
		    (empty & 1 << ring->ring_number) >> ring->ring_number);
		sbuf_printf(&sb, "ring size %d, msg size %d\n",
...
adf_ring_debugfs_add(struct adf_etr_ring_data *ring, const char *name)
{
	...
	ring_debug->debug = SYSCTL_ADD_PROC(&ring->bank->accel_dev->sysctl_ctx,
	    SYSCTL_CHILDREN(ring->bank->bank_debug_dir), OID_AUTO, entry_name,
	    CTLFLAG_RD | CTLTYPE_STRING, ring, 0, adf_ring_show, "A",
	    "Ring configuration");
...
static int
adf_bank_show(SYSCTL_HANDLER_ARGS)
{
...
adf_bank_debugfs_add(struct adf_etr_bank_data *bank)
{
	...
	bank->bank_debug_dir = SYSCTL_ADD_NODE(&accel_dev->sysctl_ctx,
	    SYSCTL_CHILDREN(parent), OID_AUTO, name, CTLFLAG_RD | CTLFLAG_SKIP,
	    NULL, "");
	...
	bank->bank_debug_cfg = SYSCTL_ADD_PROC(&accel_dev->sysctl_ctx,
	    SYSCTL_CHILDREN(bank->bank_debug_dir), OID_AUTO, "config",
	    CTLFLAG_RD | CTLTYPE_STRING, bank, 0, adf_bank_show, "A",
	    "Bank configuration");
