...
int
enic_get_vnic_config(struct enic *enic)
{
...
#define GET_CONFIG(m)                                                         \
	do {                                                                  \
		err = vnic_dev_spec(enic->vdev,                               \
		    offsetof(struct vnic_enet_config, m), sizeof(c->m),       \
		    &c->m);                                                   \
		if (err) {                                                    \
	...
	c->wq_desc_count = min_t(u32, ENIC_MAX_WQ_DESCS,
	    max_t(u32, ENIC_MIN_WQ_DESCS, c->wq_desc_count));
	c->wq_desc_count &= 0xffffffe0; /* must be aligned to groups of 32 */

	c->rq_desc_count = min_t(u32, ENIC_MAX_RQ_DESCS,
...
int
enic_add_vlan(struct enic *enic, u16 vlanid)
{
	u64 a0 = vlanid, a1 = 0;
	int wait = 1000;
	int err;
...
int
enic_del_vlan(struct enic *enic, u16 vlanid)
{
	u64 a0 = vlanid, a1 = 0;
	int wait = 1000;
	int err;
...
int
enic_set_nic_cfg(struct enic *enic, u8 rss_default_cpu, u8 rss_hash_type,
    ...
    u8 ig_vlan_strip_en)
{
	...
	vnic_set_nic_cfg(&nic_cfg, rss_default_cpu, rss_hash_type,
	    rss_hash_bits, rss_base_cpu, rss_enable, tso_ipid_split_en,
	    ig_vlan_strip_en);
...
void
enic_get_res_counts(struct enic *enic)
{
