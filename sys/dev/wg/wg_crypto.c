 ...
 * Reserved. Copyright (c) 2022 The FreeBSD Foundation
 */
...
static inline void
cpu_to_le32_array(uint32_t *buf, unsigned int words)
{
	while (words--) {
		*buf = cpu_to_le32(*buf);
		++buf;
...
static inline void
le32_to_cpu_array(uint32_t *buf, unsigned int words)
{
	while (words--) {
...
static inline void
blake2s_compress(struct blake2s_state *state, const uint8_t *block,
...
crypto_init(void)
{
	struct crypto_session_params csp = { .csp_mode = CSP_MODE_AEAD,
