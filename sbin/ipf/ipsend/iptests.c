...
void ip_test2(dev, mtu, ip, gwip, ptest) char *dev;
int mtu;
ip_t *ip;
struct in_addr gwip;
int ptest;
...
ip_test3(char *dev, int mtu, ip_t *ip, struct in_addr gwip, int ptest)
{
	...
	if (!ptest || (ptest == 5)) {
		...
		for (i = 0; ict1[i]; i++) {
			...
			(void)send_icmp(nfd, mtu, ip, gwip);
			PAUSE();
			printf("3.5.%d ICMP type 5 code 0 (all 0's)\r", i * 4);
			icp->icmp_code = 127;
			(void)send_icmp(nfd, mtu, ip, gwip);
			PAUSE();
	...
	if (!ptest || (ptest == 7)) {
		...
		for (i = 0; ict2[i]; i++) {
			...
			(void)send_icmp(nfd, mtu, ip, gwip);
			PAUSE();
			printf("3.5.%d ICMP type %d code 0 (all 0's)\r", i * 4,
			    icp->icmp_type);
			icp->icmp_code = 127;
			(void)send_icmp(nfd, mtu, ip, gwip);
			PAUSE();
...
ip_test5(char *dev, int mtu, ip_t *ip, struct in_addr gwip, int ptest)
{
    ...
    !defined(__sgi) && !defined(__hpux) && !defined(__osf__)
	{
		...
		if (!ptest || (ptest == 5)) {
			...
			printf(
			    "5.5.1 TCP Urgent pointer, sport %hu dport %hu\n",
		...
		if (!ptest || (ptest == 6)) {
			/*
			 * Test 6: data offset, off = 0, off is inside, off is
			 * outside
			 */
			t->th_flags = TH_ACK;
