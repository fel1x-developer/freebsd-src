...
send_udp(int nfd, int mtu, ip_t *ip, struct in_addr gwip)
{
	...
	ti->ti_src = ip->ip_src;
	ti->ti_dst = ip->ip_dst;
	bcopy((char *)ip + (IP_HL(ip) << 2), (char *)&ti->ti_sport,
	    sizeof(udphdr_t));
	...
	bcopy((char *)&ti->ti_sport, (char *)ip + (IP_HL(ip) << 2),
	    sizeof(udphdr_t));
	return (send_ip(nfd, mtu, ip, gwip, 1));
}
...
int send_packet(int nfd, int mtu, ip_t *ip, struct in_addr gwip) int nfd, mtu;
ip_t *ip;
struct in_addr gwip;
{
	switch (ip->ip_p) {
