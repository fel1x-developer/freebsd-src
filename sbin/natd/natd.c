...
static int StrToPortRange(const char *str, const char *proto,
    port_range *portRange);
static int StrToProto(const char *str);
static int StrToAddrAndPortRange(char *str, struct in_addr *addr, char *proto,
    port_range *portRange);
...
main(int argc, char **argv)
{
	...
	LIST_FOREACH (mip, &root, list) {
		...
		if (mip->aliasAddr.s_addr == INADDR_NONE &&
			...
			errx(1, "instance %s: aliasing address not given",
			    mip->name);

		if (mip->aliasAddr.s_addr != INADDR_NONE && mip->ifName != NULL)
			errx(1,
			    "both alias address and interface "
		...
		if (mip->inPort != 0 || mip->outPort != 0)
			if (mip->inPort == 0 || mip->outPort == 0)
				errx(1,
				    "both input and output ports are required");
		...
		/*
		 * Create divert sockets. Use only one socket if -p was
		 * specified on command line. Otherwise, create separate sockets
		...
		if (mip->inOutPort) {
			...
			if (bind(mip->divertInOut, (struct sockaddr *)&addr,
				sizeof addr) == -1)
			...
			if (bind(mip->divertIn, (struct sockaddr *)&addr,
				sizeof addr) == -1)
			...
			if (bind(mip->divertOut, (struct sockaddr *)&addr,
				sizeof addr) == -1)
				Quit("Unable to bind outgoing divert socket.");
		}
		/*
		 * Create routing socket if interface name specified and in
		 * dynamic mode.
		 */
		if (mip->ifName) {
			if (dynamicMode) {
				...
				if (routeSock == -1)
					routeSock = socket(PF_ROUTE, SOCK_RAW,
					    0);
				if (routeSock == -1)
					Quit(
					    "Unable to create routing info socket.");
				if (routeSock > fdMax)
					fdMax = routeSock;
	...
	if (globalPort) {
		...
		if (bind(divertGlobal, (struct sockaddr *)&addr, sizeof addr) ==
		    -1)
...
ParseArgs(int argc, char **argv)
{
	...
	for (arg = 1; arg < argc; arg++) {
		...
		while (arg < argc - 1) {
			...
			if (len) {
				strncat(parmBuf, " ",
				    sizeof(parmBuf) - (len + 1));
				len += strlen(parmBuf + len);
			}

			++arg;
			strncat(parmBuf, argv[arg],
			    sizeof(parmBuf) - (len + 1));
			len += strlen(parmBuf + len);
		}
...
DoGlobal(int fd)
{
	...
	origBytes = recvfrom(fd, buf, sizeof buf, 0, (struct sockaddr *)&addr,
	    &addrSize);
	...
	LIST_FOREACH (mip, &root, list) {
		mla = mip->la;
		if (LibAliasOutTry(mla, buf, IP_MAXPACKET, 0) !=
		    PKT_ALIAS_IGNORED)
			break;
	}
	/*
	 ...
	 */
	wrote = sendto(fd, buf, bytes, 0, (struct sockaddr *)&addr,
	    sizeof addr);
	...
	if (wrote != bytes) {
		...
		if (errno == EMSGSIZE && mip != NULL) {
			...
			if (mip->ifMTU != -1)
				SendNeedFragIcmp(icmpSock, (struct ip *)buf,
...
DoAliasing(int fd, int direction)
{
	...
	if (direction == OUTPUT) {
		...
		if (status == PKT_ALIAS_IGNORED && mip->dropIgnoredIncoming) {
	 ...
	 * Update alias overhead size for outgoing packets.
	 */
	if (direction == OUTPUT && bytes - origBytes > mip->aliasOverhead)
		mip->aliasOverhead = bytes - origBytes;
	...
	if (wrote != bytes) {
		...
		if (errno == EMSGSIZE) {
			...
			if (direction == OUTPUT && mip->ifMTU != -1)
				SendNeedFragIcmp(icmpSock, (struct ip *)buf,
				    mip->ifMTU - mip->aliasOverhead);
		} else if (errno == EACCES && logIpfwDenied) {
...
FormatPacket(struct ip *ip)
{
	...
	case IPPROTO_TCP:
		...
		sprintf(buf, "[TCP] %s:%d -> %s:%d", src,
		    ntohs(tcphdr->th_sport), dst, ntohs(tcphdr->th_dport));
		break;

	case IPPROTO_UDP:
		udphdr = (struct udphdr *)((char *)ip + (ip->ip_hl << 2));
		sprintf(buf, "[UDP] %s:%d -> %s:%d", src,
		    ntohs(udphdr->uh_sport), dst, ntohs(udphdr->uh_dport));
		break;

	case IPPROTO_ICMP:
		icmphdr = (struct icmp *)((char *)ip + (ip->ip_hl << 2));
		sprintf(buf, "[ICMP] %s -> %s %u(%u)", src, dst,
...
static struct OptionInfo optionTable[] = {
	...
	{ LibAliasOption, PKT_ALIAS_UNREGISTERED_ONLY, YesNo, "[yes|no]",
	    "alias only unregistered addresses", "unregistered_only", "u" },

	{ LibAliasOption, PKT_ALIAS_LOG, YesNo, "[yes|no]", "enable logging",
	    "log", "l" },

	{ LibAliasOption, PKT_ALIAS_PROXY_ONLY, YesNo, "[yes|no]", "proxy only",
	    "proxy_only", NULL },

	{ LibAliasOption, PKT_ALIAS_REVERSE, YesNo, "[yes|no]",
	    "operate in reverse mode", "reverse", NULL },

	{ LibAliasOption, PKT_ALIAS_DENY_INCOMING, YesNo, "[yes|no]",
	    "allow incoming connections", "deny_incoming", "d" },

	{ LibAliasOption, PKT_ALIAS_USE_SOCKETS, YesNo, "[yes|no]",
	    "use sockets to inhibit port conflict", "use_sockets", "s" },

	{ LibAliasOption, PKT_ALIAS_SAME_PORTS, YesNo, "[yes|no]",
	    "try to keep original port numbers for connections", "same_ports",
	    "m" },

	{ Verbose, 0, YesNo, "[yes|no]",
	    "verbose mode, dump packet information", "verbose", "v" },

	{ DynamicMode, 0, YesNo, "[yes|no]",
	    "dynamic mode, automatically detect interface address changes",
	    "dynamic", NULL },

	{ InPort, 0, Service, "number|service_name",
	    "set port for incoming packets", "in_port", "i" },

	{ OutPort, 0, Service, "number|service_name",
	    "set port for outgoing packets", "out_port", "o" },

	{ Port, 0, Service, "number|service_name",
	    "set port (defaults to natd/divert)", "port", "p" },

	{ GlobalPort, 0, Service, "number|service_name", "set globalport",
	    "globalport", NULL },

	{ AliasAddress, 0, Address, "x.x.x.x", "address to use for aliasing",
	    "alias_address", "a" },

	{ TargetAddress, 0, Address, "x.x.x.x",
	    "address to use for incoming sessions", "target_address", "t" },

	{ InterfaceName, 0, String, "network_if_name",
	    "take aliasing address from interface", "interface", "n" },

	{ ProxyRule, 0, String,
	    "[type encode_ip_hdr|encode_tcp_stream] port xxxx server "
	    "a.b.c.d:yyyy",
	    "add transparent proxying / destination NAT", "proxy_rule", NULL },

	{ RedirectPort, 0, String,
	    "tcp|udp local_addr:local_port_range[,...] [public_addr:]public_port_range"
	    " [remote_addr[:remote_port_range]]",
	    "redirect a port (or ports) for incoming traffic", "redirect_port",
	    NULL },

	{ RedirectProto, 0, String,
	    "proto local_addr [public_addr] [remote_addr]",
	    "redirect packets of a given proto", "redirect_proto", NULL },

	{ RedirectAddress, 0, String, "local_addr[,...] public_addr",
	    "define mapping between local and public addresses",
	    "redirect_address", NULL },

	{ ConfigFile, 0, String, "file_name",
	    "read options from configuration file", "config", "f" },

	{ LogDenied, 0, YesNo, "[yes|no]",
	    "enable logging of denied incoming packets", "log_denied", NULL },

	{ LogFacility, 0, String, "facility",
	    "name of syslog facility to use for logging", "log_facility",
	    NULL },

	{ PunchFW, 0, String, "basenumber:count",
	    "punch holes in the firewall for incoming FTP/IRC DCC connections",
	    "punch_fw", NULL },

	{ SkinnyPort, 0, String, "port",
	    "set the TCP port for use with the Skinny Station protocol",
	    "skinny_port", NULL },

	{ LogIpfwDenied, 0, YesNo, "[yes|no]",
	    "log packets converted by natd, but denied by ipfw",
	    "log_ipfw_denied", NULL },

	{ PidFile, 0, String, "file_name", "store PID in an alternate file",
	    "pid_file", "P" },
	{ Instance, 0, String, "instance name",
	    "name of aliasing engine instance", "instance", NULL },
	{ ExitDelay, 0, Numeric, "ms",
	    "delay in ms before daemon exit after signal", "exit_delay", NULL },
};
...
SetupPortRedirect(const char *parms)
{
	...
	if (separator) { /* LSNAT redirection syntax. */
		...
		if (StrToAddrAndPortRange(ptr, &localAddr, protoName,
			&portRange) != 0)
	...
	separator = strchr(ptr, ':');
	if (separator) {
		if (StrToAddrAndPortRange(ptr, &publicAddr, protoName,
			&portRange) != 0)
	...
	if (ptr) {
		separator = strchr(ptr, ':');
		if (separator) {
			if (StrToAddrAndPortRange(ptr, &remoteAddr, protoName,
				&portRange) != 0)
				errx(1,
				    "redirect_port: invalid remote port range");
		} else {
	...
	if (numRemotePorts != numLocalPorts &&
	    (numRemotePorts != 1 || remotePort != 0))
		errx(1,
		    "redirect_port: remote port must be 0 or equal to local port range in size");
	...
	for (i = 0; i < numPublicPorts; ++i) {
		...
		aliaslink = LibAliasRedirectPort(mla, localAddr,
		    htons(localPort + i), remoteAddr, htons(remotePortCopy),
		    publicAddr, htons(publicPort + i), proto);
	}
	...
	if (serverPool != NULL && aliaslink != NULL) {
		...
		while (ptr != NULL) {
			if (StrToAddrAndPortRange(ptr, &localAddr, protoName,
				&portRange) != 0)
				errx(1,
				    "redirect_port: invalid local port range");

			localPort = GETLOPORT(portRange);
			if (GETNUMPORTS(portRange) != 1)
				errx(1,
				    "redirect_port: local port must be single in this context");
			LibAliasAddServer(mla, aliaslink, localAddr,
			    htons(localPort));
			ptr = strtok(NULL, ",");
		}
	}
...
SetupPunchFW(const char *strValue)
{
	...
	if (CheckIpfwRulenum(base + num - 1) == -1)
		errx(1,
		    "punch_fw: basenumber:count parameter should fit "
