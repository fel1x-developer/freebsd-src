...
main(int argc, char **argv)
{
	...
	if (argc > 1)
		NOPS = strtoul(argv[1], 0, 0);
	if (argc > 2)
		NBUCKETS = strtoul(argv[2], 0, 0);
	if (argc > 3)
		NSIZE = strtoul(argv[3], 0, 0);
	...
	for (i = 0; i < NOPS; i++) {
		...
		if (k & 1 || k == 0) {
			/*
			 * Workaround because realloc return bogus pointer
			 * rather than NULL if passed zero length.
			 */
			foo[j] = NULL;
		}
		if (foo[j])
