...
static void
table_offset_add(struct table *t, struct offset *offset)
{
...
static int
parser_auth(struct table *t, const char *filename)
{
	FILE *file;
	regex_t mask_rex;
	regmatch_t match[4];
	...
	if (regcomp(&mask_rex, "(0x[0-9a-fA-F]*) *([_a-zA-Z0-9]*)",
		REG_EXTENDED)) {
