...
#define FOREACH_OMPT_INQUIRY_FN(macro)                                    \
	macro(ompt_enumerate_states) macro(ompt_enumerate_mutex_impls)    \
		...
		macro(ompt_get_state)                                     \
                                                                          \
		    macro(ompt_get_parallel_info) macro(                  \
			ompt_get_task_info) macro(ompt_get_task_memory)   \
			macro(ompt_get_thread_data) macro(                \
			    ompt_get_unique_id) macro(ompt_finalize_tool) \
                                                                          \
			    macro(ompt_get_num_procs)                     \
				macro(ompt_get_num_places) macro(         \
				    ompt_get_place_proc_ids)              \
				    macro(ompt_get_place_num) macro(      \
...
#define FOREACH_OMPT_STATE(macro)                                              \
	...
	macro(ompt_state_undefined, 0x102) /* undefined thread state */        \
                                                                               \
	    /* work states (0..15) */                                          \
	    macro(ompt_state_work_serial,                                      \
		0x000) /* working outside parallel */                          \
	    macro(ompt_state_work_parallel,                                    \
		0x001) /* working within parallel */                           \
	    macro(ompt_state_work_reduction,                                   \
		0x002) /* performing a reduction */                            \
                                                                               \
	    ...
	    macro(ompt_state_wait_barrier_implicit,                            \
		0x013) /* implicit barrier */                                  \
	    macro(ompt_state_wait_barrier_explicit,                            \
		0x014) /* explicit barrier */                                  \
                                                                               \
	    /* task wait states (32..63) */                                    \
	    macro(ompt_state_wait_taskwait, 0x020) /* waiting at a taskwait */ \
	    macro(ompt_state_wait_taskgroup,                                   \
		0x021) /* waiting at a taskgroup */                            \
                                                                               \
	    ...
	    macro(ompt_state_wait_target,                                      \
		0x080) /* waiting for target region */                         \
	    ...
	    macro(ompt_state_overhead,                                         \
		0x101) /* overhead excluding wait states */                    \
                                                                               \
...
#define FOREACH_KMP_MUTEX_IMPL(macro)                                        \
	macro(kmp_mutex_impl_none, 0)	     /* unknown implementation */    \
	    ...
	    macro(kmp_mutex_impl_speculative,                                \
		3) /* based on HW-supported speculation */
...
#define FOREACH_OMPT_HOST_EVENT(macro)                                         \
	...
	macro(ompt_callback_thread_begin, ompt_callback_thread_begin_t,        \
		...
		ompt_callback_parallel_begin_t, 3) /* parallel begin */                                                                            \
		...
		17) /* mutex released                  */                      \
                                                                               \
	    macro(ompt_callback_dependences, ompt_callback_dependences_t,      \
		18) /* report task dependences         */                      \
	    macro(ompt_callback_task_dependence,                               \
		...
		20) /* task at work begin or end       */                      \
                                                                               \
	    macro(ompt_callback_masked, ompt_callback_masked_t,                \
		21) /* task at masked begin or end     */                      \
                                                                               \
		...
		30) /* cancel innermost binding region */                      \
                                                                               \
	    macro(ompt_callback_reduction, ompt_callback_sync_region_t,        \
		31) /* reduction                       */                      \
                                                                               \
	    macro(ompt_callback_dispatch, ompt_callback_dispatch_t,            \
		32) /* dispatch of work                */                      \
...
#define FOREACH_OMPT_DEVICE_EVENT(macro)                                      \
	...
	macro(ompt_callback_device_initialize,                                \
	    ompt_callback_device_initialize_t, 12) /* device initialize */                                                                           \
	    macro(ompt_callback_device_finalize,                              \
		ompt_callback_device_finalize_t, 13) /* device finalize */                                                                           \
                                                                              \
...
#define FOREACH_OMPT_NOEMI_EVENT(macro)                                       \
	...
	macro(ompt_callback_target, ompt_callback_target_t, 8) /* target */                                                                           \
		...
		ompt_callback_target_data_op_t, 9) /* target data op */                                                                           \
...
#define FOREACH_OMPT_EMI_EVENT(macro)                                        \
	/*--- Mandatory Events ---*/                                         \
	macro(ompt_callback_target_emi, ompt_callback_target_emi_t,          \
	    33) /* target                          */                        \
	    macro(ompt_callback_target_data_op_emi,                          \
		ompt_callback_target_data_op_emi_t, 34) /* target data op */                                                                          \
		...
		ompt_callback_target_map_emi_t, 36) /* target map */
...
typedef ompt_set_result_t (
    *ompt_set_callback_t)(ompt_callbacks_t event, ompt_callback_t callback);

typedef int (
    *ompt_get_callback_t)(ompt_callbacks_t event, ompt_callback_t *callback);
...
typedef int (
    *ompt_get_partition_place_nums_t)(int place_nums_size, int *place_nums);
...
typedef double (
    *ompt_translate_time_t)(ompt_device_t *device, ompt_device_time_t time);

typedef ompt_set_result_t (*ompt_set_trace_ompt_t)(ompt_device_t *device,
    unsigned int enable, unsigned int etype);

typedef ompt_set_result_t (
    *ompt_set_trace_native_t)(ompt_device_t *device, int enable, int flags);
...
typedef int (*ompt_advance_buffer_cursor_t)(ompt_device_t *device,
    ompt_buffer_t *buffer, size_t size, ompt_buffer_cursor_t current,
...
typedef void *(*ompt_get_record_native_t)(ompt_buffer_t *buffer,
    ompt_buffer_cursor_t current, ompt_id_t *host_op_id);
...
typedef void (
    *ompt_callback_parallel_begin_t)(ompt_data_t *encountering_task_data,
...
typedef void (*ompt_callback_parallel_end_t)(ompt_data_t *parallel_data,
    ompt_data_t *encountering_task_data, int flags, const void *codeptr_ra);
...
typedef void (*ompt_callback_task_schedule_t)(ompt_data_t *prior_task_data,
    ompt_task_status_t prior_task_status, ompt_data_t *next_task_data);
...
typedef void (
    *ompt_callback_flush_t)(ompt_data_t *thread_data, const void *codeptr_ra);
...
typedef void (*ompt_callback_device_load_t)(int device_num,
    ...
    size_t bytes, void *host_addr, void *device_addr, uint64_t module_id);

typedef void (
    *ompt_callback_device_unload_t)(int device_num, uint64_t module_id);

typedef void (
    *ompt_callback_target_data_op_emi_t)(ompt_scope_endpoint_t endpoint,
...
typedef void (*ompt_callback_target_map_emi_t)(ompt_data_t *target_data,
...
typedef void (
    *ompt_callback_target_submit_emi_t)(ompt_scope_endpoint_t endpoint,
...
typedef ompd_rc_t (
    *ompd_callback_memory_alloc_fn_t)(ompd_size_t nbytes, void **ptr);
...
typedef ompd_rc_t (
    *ompd_callback_print_string_fn_t)(const char *string, int category);
