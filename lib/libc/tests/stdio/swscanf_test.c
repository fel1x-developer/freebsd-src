...
static const struct swscanf_test_case {
	...
	//	input		binary		octal		decimal
	//hexadecimal	automatic
	// all digits
	{
	    L"0",
	    { 1, 0, 1 },
...
ATF_TC_BODY(swscanf_b, tc)
{
	...
	for (stc = swscanf_test_cases; *stc->input; stc++) {
		...
		SWSCANF_TEST(input + 1, L"%b", stc->b.ret, stc->b.val,
		    stc->b.len);
		input[0] = L'+';
		SWSCANF_TEST(input, L"%b", stc->b.ret, stc->b.val,
		    stc->b.len ? stc->b.len + 1 : 0);
		input[0] = L'-';
		SWSCANF_TEST(input, L"%b", stc->b.ret, -stc->b.val,
		    stc->b.len ? stc->b.len + 1 : 0);
	}
...
ATF_TC_BODY(swscanf_o, tc)
{
	...
	for (stc = swscanf_test_cases; *stc->input; stc++) {
		wcscpy(input + 1, stc->input);
		SWSCANF_TEST(input + 1, L"%o", stc->o.ret, stc->o.val,
		    stc->o.len);
		input[0] = L'+';
		SWSCANF_TEST(input, L"%o", stc->o.ret, stc->o.val,
		    stc->o.len ? stc->o.len + 1 : 0);
		input[0] = L'-';
		SWSCANF_TEST(input, L"%o", stc->o.ret, -stc->o.val,
		    stc->o.len ? stc->o.len + 1 : 0);
	}
...
ATF_TC_BODY(swscanf_d, tc)
{
	...
	for (stc = swscanf_test_cases; *stc->input; stc++) {
		wcscpy(input + 1, stc->input);
		SWSCANF_TEST(input + 1, L"%d", stc->d.ret, stc->d.val,
		    stc->d.len);
		input[0] = L'+';
		SWSCANF_TEST(input, L"%d", stc->d.ret, stc->d.val,
		    stc->d.len ? stc->d.len + 1 : 0);
		input[0] = L'-';
		SWSCANF_TEST(input, L"%d", stc->d.ret, -stc->d.val,
		    stc->d.len ? stc->d.len + 1 : 0);
	}
...
ATF_TC_BODY(swscanf_x, tc)
{
	...
	for (stc = swscanf_test_cases; *stc->input; stc++) {
		wcscpy(input + 1, stc->input);
		SWSCANF_TEST(input + 1, L"%x", stc->x.ret, stc->x.val,
		    stc->x.len);
		input[0] = L'+';
		SWSCANF_TEST(input, L"%x", stc->x.ret, stc->x.val,
		    stc->x.len ? stc->x.len + 1 : 0);
		input[0] = L'-';
		SWSCANF_TEST(input, L"%x", stc->x.ret, -stc->x.val,
		    stc->x.len ? stc->x.len + 1 : 0);
	}
...
ATF_TC_BODY(swscanf_i, tc)
{
	...
	for (stc = swscanf_test_cases; *stc->input; stc++) {
		wcscpy(input + 1, stc->input);
		SWSCANF_TEST(input + 1, L"%i", stc->i.ret, stc->i.val,
		    stc->i.len);
		input[0] = L'+';
		SWSCANF_TEST(input, L"%i", stc->i.ret, stc->i.val,
		    stc->i.len ? stc->i.len + 1 : 0);
		input[0] = L'-';
		SWSCANF_TEST(input, L"%i", stc->i.ret, -stc->i.val,
		    stc->i.len ? stc->i.len + 1 : 0);
	}
