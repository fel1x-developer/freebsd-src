...
*/
static void
normalizeFloat64Subnormal(bits32 aSig0, bits32 aSig1, int16 *zExpPtr,
	...
	if (0x7FD <= (bits16)zExp) {
		if ((0x7FD < zExp) ||
			...
			if ((roundingMode == float_round_to_zero) ||
				...
				return packFloat64(zSign, 0x7FE, 0x000FFFFF,
				    0xFFFFFFFF);
			}
		...
		if (zExp < 0) {
			...
			shift64ExtraRightJamming(zSig0, zSig1, zSig2, -zExp,
			    &zSig0, &zSig1, &zSig2);
			zExp = 0;
			if (isTiny && zSig2)
				float_raise(float_flag_underflow);
...
normalizeRoundAndPackFloat64(flag zSign, int16 zExp, bits32 zSig0, bits32 zSig1)
{
	...
	if (0 <= shiftCount) {
		...
		shift64ExtraRightJamming(zSig0, zSig1, 0, -shiftCount, &zSig0,
		    &zSig1, &zSig2);
	}
	zExp -= shiftCount;
...
float64_to_int32(float64 a)
{
	...
	if (0 <= shiftCount) {
		...
		shortShift64Left(aSig0 | 0x00100000, aSig1, shiftCount, &absZ,
		    &aSigExtra);
		if (0x80000000 < absZ)
			goto invalid;
	} else {
...
float64_round_to_int(float64 a)
{
	...
	if (0x413 <= aExp) {
		...
		if (roundingMode == float_round_nearest_even) {
			if (lastBitMask) {
				add64(z.high, z.low, 0, lastBitMask >> 1,
				    &z.high, &z.low);
			...
			if (extractFloat64Sign(z) ^
				...
				add64(z.high, z.low, 0, roundBitsMask, &z.high,
				    &z.low);
			}
		}
		z.low &= ~roundBitsMask;
...
addFloat64Sigs(float64 a, float64 b, flag zSign)
{
	...
	if (0 < expDiff) {
		...
		shift64ExtraRightJamming(bSig0, bSig1, 0, expDiff, &bSig0,
		    &bSig1, &zSig2);
		zExp = aExp;
	} else if (expDiff < 0) {
		if (bExp == 0x7FF) {
		...
		shift64ExtraRightJamming(aSig0, aSig1, 0, -expDiff, &aSig0,
		    &aSig1, &zSig2);
		zExp = bExp;
	} else {
		if (aExp == 0x7FF) {
	...
	++zExp;
shiftRight1:
	shift64ExtraRightJamming(zSig0, zSig1, zSig2, 1, &zSig0, &zSig1,
	    &zSig2);
roundAndPack:
	return roundAndPackFloat64(zSign, zExp, zSig0, zSig1, zSig2);
}
...
float64_div(float64 a, float64 b)
{
	...
	if ((zSig1 & 0x3FF) <= 4) {
		...
		while ((sbits32)rem1 < 0) {
			--zSig1;
			add96(rem1, rem2, rem3, 0, bSig0, bSig1, &rem1, &rem2,
			    &rem3);
		}
...
float64_rem(float64 a, float64 b)
{
	...
	if (expDiff < -1)
		return a;
	shortShift64Left(aSig0 | 0x00100000, aSig1, 11 - (expDiff < 0), &aSig0,
	    &aSig1);
	shortShift64Left(bSig0 | 0x00100000, bSig1, 11, &bSig0, &bSig1);
	...
	while (0 < expDiff) {
		...
		shortShift96Left(term0, term1, term2, 29, &term1, &term2,
		    &allZero);
	...
	add64(aSig0, aSig1, alternateASig0, alternateASig1, &sigMean0,
	    &sigMean1);
	...
	return normalizeRoundAndPackFloat64(aSign ^ zSign, bExp - 4, aSig0,
	    aSig1);
}
#endif
...
float64_sqrt(float64 a)
{
	...
	if ((zSig1 & 0x1FF) <= 5) {
		...
		while ((sbits32)rem1 < 0) {
			...
			term3 |= 1;
			term2 |= doubleZSig0;
			add96(rem1, rem2, rem3, 0, term2, term3, &rem1, &rem2,
			    &rem3);
		}
		zSig1 |= ((rem1 | rem2 | rem3) != 0);
	}
