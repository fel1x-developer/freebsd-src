	...
	if (!roundNearestEven) {
		if (roundingMode == float_round_to_zero) {
			...
			if (zSign) {
				...
				increment = (roundingMode == float_round_up) &&
				    absZ1;
			}
		}
	}
...
*/
static floatx80
roundAndPackFloatx80(int8 roundingPrecision, flag zSign, int32 zExp,
...
precision80:
	...
	if (0x7FFD <= (bits32)(zExp - 1)) {
		...
		if (zExp <= 0) {
			...
			shift64ExtraRightJamming(zSig0, zSig1, 1 - zExp, &zSig0,
			    &zSig1);
			zExp = 0;
			if (isTiny && zSig1)
				float_raise(float_flag_underflow);
...
*/
static floatx80
normalizeRoundAndPackFloatx80(int8 roundingPrecision, flag zSign, int32 zExp,
    bits64 zSig0, bits64 zSig1)
{
	int8 shiftCount;
	...
	zExp -= shiftCount;
	return roundAndPackFloatx80(roundingPrecision, zSign, zExp, zSig0,
	    zSig1);
}
...
*/
static void
normalizeFloat128Subnormal(bits64 aSig0, bits64 aSig1, int32 *zExpPtr,
...
*/
static float128
roundAndPackFloat128(flag zSign, int32 zExp, bits64 zSig0, bits64 zSig1,
    bits64 zSig2)
{
	int8 roundingMode;
	flag roundNearestEven, increment, isTiny;
	...
	if (0x7FFD <= (bits32)zExp) {
		if ((0x7FFD < zExp) ||
			...
			if ((roundingMode == float_round_to_zero) ||
			    (zSign && (roundingMode == float_round_up)) ||
			    (!zSign && (roundingMode == float_round_down))) {
				return packFloat128(zSign, 0x7FFE,
		...
		if (zExp < 0) {
			isTiny = (float_detect_tininess ==
				...
				LIT64(0xFFFFFFFFFFFFFFFF));
			shift128ExtraRightJamming(zSig0, zSig1, zSig2, -zExp,
			    &zSig0, &zSig1, &zSig2);
			zExp = 0;
			if (isTiny && zSig2)
				float_raise(float_flag_underflow);
...
*/
static float128
normalizeRoundAndPackFloat128(flag zSign, int32 zExp, bits64 zSig0,
    bits64 zSig1)
{
	int8 shiftCount;
	bits64 zSig2;
	...
	if (0 <= shiftCount) {
		...
		shift128ExtraRightJamming(zSig0, zSig1, 0, -shiftCount, &zSig0,
		    &zSig1, &zSig2);
	}
	zExp -= shiftCount;
...
float64_to_int64(float64 a)
{
	...
	if (shiftCount <= 0) {
		...
		shift64ExtraRightJamming(aSig, 0, shiftCount, &aSig,
		    &aSigExtra);
	}
...
float64_sqrt(float64 a)
{
	...
	if ((zSig & 0x1FF) <= 5) {
		...
		while ((sbits64)rem0 < 0) {
			--zSig;
			doubleZSig -= 2;
			add128(rem0, rem1, zSig >> 63, doubleZSig | 1, &rem0,
			    &rem1);
		}
		zSig |= ((rem0 | rem1) != 0);
	}
...
bExpBigger:
	if (bExp == 0x7FFF) {
		if ((bits64)(bSig << 1))
			return propagateFloatx80NaN(a, b);
		return packFloatx80(zSign ^ 1, 0x7FFF,
		    LIT64(0x8000000000000000));
	}
	if (aExp == 0)
		++expDiff;
...
aBigger:
	...
	zExp = aExp;
normalizeRoundAndPack:
	return normalizeRoundAndPackFloatx80(floatx80_rounding_precision, zSign,
	    zExp, zSig0, zSig1);
}
...
floatx80_sqrt(floatx80 a)
{
	...
	if ((zSig1 & LIT64(0x3FFFFFFFFFFFFFFF)) <= 5) {
		...
		while ((sbits64)rem1 < 0) {
			...
			term3 |= 1;
			term2 |= doubleZSig0;
			add192(rem1, rem2, rem3, 0, term2, term3, &rem1, &rem2,
			    &rem3);
		}
...
#endif /* (SOFTFLOATSPARC64_FOR_GCC || SOFTFLOAT_FOR_GCC) && \
...
float128_round_to_int(float128 a)
{
	...
	if (0x402F <= aExp) {
		...
		if (roundingMode == float_round_nearest_even) {
			if (lastBitMask) {
				add128(z.high, z.low, 0, lastBitMask >> 1,
				    &z.high, &z.low);
				if ((z.low & roundBitsMask) == 0)
					z.low &= ~lastBitMask;
			} else {
			...
			if (extractFloat128Sign(z) ^
				...
				add128(z.high, z.low, 0, roundBitsMask, &z.high,
				    &z.low);
			}
		}
		z.low &= ~roundBitsMask;
		...
		if (aExp < 0x3FFF) {
			...
			case float_round_nearest_even:
				if ((aExp == 0x3FFE) &&
				    (extractFloat128Frac0(a) |
					extractFloat128Frac1(a))) {
					return packFloat128(aSign, 0x3FFF, 0,
					    0);
				}
				break;
			case float_round_to_zero:
...
addFloat128Sigs(float128 a, float128 b, flag zSign)
{
	...
	if (0 < expDiff) {
		...
		shift128ExtraRightJamming(bSig0, bSig1, 0, expDiff, &bSig0,
		    &bSig1, &zSig2);
		zExp = aExp;
	} else if (expDiff < 0) {
		if (bExp == 0x7FFF) {
		...
		shift128ExtraRightJamming(aSig0, aSig1, 0, -expDiff, &aSig0,
		    &aSig1, &zSig2);
		zExp = bExp;
	} else {
		if (aExp == 0x7FFF) {
	...
	++zExp;
shiftRight1:
	shift128ExtraRightJamming(zSig0, zSig1, zSig2, 1, &zSig0, &zSig1,
	    &zSig2);
roundAndPack:
	return roundAndPackFloat128(zSign, zExp, zSig0, zSig1, zSig2);
}
...
float128_mul(float128 a, float128 b)
{
	...
	if (LIT64(0x0002000000000000) <= zSig0) {
		shift128ExtraRightJamming(zSig0, zSig1, zSig2, 1, &zSig0,
		    &zSig1, &zSig2);
		++zExp;
	}
	return roundAndPackFloat128(zSign, zExp, zSig0, zSig1, zSig2);
...
float128_div(float128 a, float128 b)
{
	...
	if ((zSig1 & 0x3FFF) <= 4) {
		...
		while ((sbits64)rem1 < 0) {
			--zSig1;
			add192(rem1, rem2, rem3, 0, bSig0, bSig1, &rem1, &rem2,
			    &rem3);
		}
		zSig1 |= ((rem1 | rem2 | rem3) != 0);
	}
...
float128_rem(float128 a, float128 b)
{
	...
	while (0 < expDiff) {
		...
		shortShift192Left(term0, term1, term2, 61, &term1, &term2,
		    &allZero);
	...
	if (-64 < expDiff) {
		...
		if (expDiff < 0) {
			...
			shortShift128Left(aSig0, aSig1, expDiff, &aSig0,
			    &aSig1);
		}
		mul128By64To192(bSig0, bSig1, q, &term0, &term1, &term2);
	...
	add128(aSig0, aSig1, alternateASig0, alternateASig1,
	    (bits64 *)&sigMean0, &sigMean1);
	...
	return normalizeRoundAndPackFloat128(aSign ^ zSign, bExp - 4, aSig0,
	    aSig1);
}
