...
be found at http://opensource.org/licenses/bsd-license.php THE PROGRAM IS
...
///
/// This identifies a signature containing a SHA-256 hash. The SignatureHeader
...
/// modulus since the public key exponent is known to be 0x10001) shall be
/// stored in big-endian order. The SignatureHeader size shall always be 0. The
/// SignatureSize shall always be 16 (size of SignatureOwner component) + 256
/// bytes.
///
...
/// hash.  The SignatureHeader size shall always be 0. The SignatureSize shall
/// always be 16 (size of SignatureOwner component) + 256 bytes.
///
...
///
/// This identifies a signature containing a SHA-1 hash.  The SignatureSize
/// shall always be 16 (size of SignatureOwner component) + 20 bytes.
///
...
///
/// This identifies a signature based on an X.509 certificate. If the signature
/// is an X.509 certificate then verification of the signature of an image
/// should validate the public key certificate in the image using certificate
/// path verification, up to this X.509 certificate as a trusted root.  The
/// SignatureHeader size shall always be 0. The SignatureSize may vary but shall
/// always be 16 (size of the SignatureOwner component) + the size of the
/// certificate itself. Note: This means that each certificate will normally be
/// in a separate EFI_SIGNATURE_LIST.
///
...
///
/// This identifies a signature containing a SHA-224 hash. The SignatureHeader
/// size shall always be 0. The SignatureSize shall always be 16 (size of
/// SignatureOwner component) + 28 bytes.
///
...
///
/// This identifies a signature containing a SHA-384 hash. The SignatureHeader
/// size shall always be 0. The SignatureSize shall always be 16 (size of
/// SignatureOwner component) + 48 bytes.
///
...
/// size shall always be 0. The SignatureSize shall always be 16 (size of
/// SignatureOwner component) + 64 bytes.
///
...
/// certificate's To-Be-Signed contents, and a time of revocation. The
/// SignatureHeader size shall always be 0. The SignatureSize shall always be 16
/// (size of the SignatureOwner component)
/// + 48 bytes for an EFI_CERT_X509_SHA256 structure. If the TimeOfRevocation is
/// non-zero, the certificate should be considered to be revoked from that time
/// and onwards, and otherwise the certificate shall be considered to always be
/// revoked.
///
...
typedef struct {
	...
	/// this is the null-terminated, user-friendly name for the device. If
	/// the image was for an application, then this is the name of the
	/// application. If this cannot be determined, then a simple NULL
	/// character should be put in this position. CHAR16 Name[];
	///

	///
	/// For device drivers, this is the device path of the device for which
	/// this device driver was intended. In some cases, the driver itself
	/// may be stored as part of the system firmware, but this field should
	/// record the device's path, not the firmware path. For applications,
	/// this is the device path of the application. If this cannot be
	/// determined, a simple end-of-path device node should be put in this
	/// position. EFI_DEVICE_PATH_PROTOCOL  DevicePath;
	///
