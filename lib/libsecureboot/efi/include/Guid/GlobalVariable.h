...
//
// Note: except the globally defined variables defined below, the spec also
// defines L"Boot####"      - A boot load option. L"Driver####"    - A driver
// load option. L"SysPrep####"   - A System Prep application load option.
// L"Key####"       - Describes hot key relationship with a Boot#### load
// option. The attribute for them is NV+BS+RT, #### is a printed hex value, and
// no 0x or h is included in the hex value. They can not be expressed as a
// #define like other globally defined variables, it is because we can not list
// the Boot0000, Boot0001, etc one by one.
//
...
/// deprecated. Its attribute is NV+BS+RT.
///
#define EFI_LANG_VARIABLE_NAME L"Lang"
///
/// The firmware's boot managers timeout, in seconds, before initiating the
/// default boot selection. Its attribute is NV+BS+RT.
///
...
///
/// The types of boot options supported by the boot manager. Should be treated
/// as read-only. Its attribute is BS+RT.
///
...
/// (0). All other values are reserved. Should be treated as read-only. Its
/// attribute is BS+RT.
///
#define EFI_SECURE_BOOT_MODE_NAME L"SecureBoot"
///
/// The OEM's default Key Exchange Key Signature Database. Should be treated as
/// read-only. Its attribute is BS+RT.
///
...
///
/// The OEM's default secure boot blacklist signature store. Should be treated
/// as read-only. Its attribute is BS+RT.
///
#define EFI_DBX_DEFAULT_VARIABLE_NAME L"dbxDefault"
///
/// The OEM's default secure boot timestamp signature store. Should be treated
/// as read-only. Its attribute is BS+RT.
///
...
///
/// Allows the OS to request the firmware to enable certain features and to take
/// certain actions. Its attribute is NV+BS+RT.
///
