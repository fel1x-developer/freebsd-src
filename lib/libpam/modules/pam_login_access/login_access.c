...
static int list_match(char *, const char *,
    int (*)(const char *, const char *, struct pam_login_access_options *),
    struct pam_login_access_options *);
    ...
    struct pam_login_access_options *login_access_opts)
{
	...
	for (tok = strtok(list, listsep); tok != NULL;
	     tok = strtok((char *)0, listsep)) {
		if (strcmp(tok, "EXCEPT") == 0) /* EXCEPT: give up */
			break;
		if ((match = (*match_fn)(tok, item, login_access_opts)) !=
		    0) /* YES */
			break;
	}
	...
	if (match != NO) {
		while ((tok = strtok((char *)0, listsep)) &&
		    strcmp(tok, "EXCEPT")) {
			/* VOID */;
		}
		if (tok == NULL ||
		    list_match((char *)0, item, match_fn, login_access_opts) ==
			NO) {
			return (match);
		}
	}
...
netgroup_match(const char *group, const char *machine, const char *user)
{
	...
	if (i == sizeof(domain)) {
		syslog(LOG_ERR,
		    "NIS netgroup support disabled: invalid NIS domain");
		return (NO);
	}
...
group_match(const char *tok, const char *username)
{
	...
	if ((group = getgrnam(tok)) == NULL) {
		if (errno != 0)
			syslog(LOG_ERR, "getgrnam() failed for %s: %s",
	...
	if ((grouplist = calloc(ngroups, sizeof(gid_t))) == NULL) {
		syslog(LOG_ERR, "cannot allocate memory for grouplist: %s",
		    username);
		return (NO);
	}
	ret = NO;
    ...
    struct pam_login_access_options *login_access_opts)
{
	...
	if (tok[0] == '@') { /* netgroup */
	    ...
	    tok[(stringlen = strlen(&tok[1]))] == ')') { /* group */
		if ((grpstr = strndup(&tok[1], stringlen - 1)) == NULL) {
			syslog(LOG_ERR, "cannot allocate memory for %s",
			    string);
			return (NO);
		}
		...
		return (YES);
	} else if (login_access_opts->defgroup ==
	    true) { /* try group membership */
    ...
    struct pam_login_access_options *login_access_opts __unused)
{
	...
	/*
	 * If a token has the magic value "ALL" the match always succeeds.
	 * Return YES if the token fully matches the string. If the token is a
	 * domain name, return YES if it matches the last fields of the string.
	 * If the token has the magic value "LOCAL", return YES if the string
	 * does not contain a "." character. If the token is a network number,
	 * return YES if it matches the head of the string.
	 */
