...
static const double zero = 0, half = 0.5, one = 1;
...
sin_pil(long double x)
{
	...
	switch (n) {
	case 0:
		y = __kernel_sinl(pi * y, zero, 0);
		break;
	case 1:
	case 2:
		y = __kernel_cosl(pi * (0.5 - y), zero);
		break;
	case 3:
	case 4:
		y = __kernel_sinl(pi * (one - y), zero, 0);
		break;
	case 5:
	case 6:
		y = -__kernel_cosl(pi * (y - 1.5), zero);
		break;
	default:
		y = __kernel_sinl(pi * (y - 2.0), zero, 0);
		break;
	}
	return -y;
}
...
lgammal_r(long double x, int *signgamp)
{
	...
	*signgamp = 1 - 2 * (hx >> 15);
	...
	/* purge 1 and 2 */
	if ((ix == 0x3fff || ix == 0x4000) && lx == 0x8000000000000000ULL)
		r = 0;
	/* for x < 2.0 */
	else if (ix < 0x4000) {
		/*
		 * XXX Supposedly, one can use the following information to
		 * replace the
		 * XXX FP rational expressions.  A similar approach is
		 * appropriate
		 * XXX for ld128, but one (may need?) needs to consider llx,
		 * too.
		 *
		...
		if (x < 8.9999961853027344e-01) {
			r = -logl(x);
			if (x >= 7.3159980773925781e-01) {
				y = 1 - x;
				i = 0;
			} else if (x >= 2.3163998126983643e-01) {
				y = x - (tc - 1);
				i = 1;
			} else {
				y = x;
			...
			r = 0;
			if (x >= 1.7316312789916992e+00) {
				y = 2 - x;
				i = 0;
			} else if (x >= 1.2316322326660156e+00) {
				y = x - tc;
				i = 1;
			} else {
				y = x - 1;
