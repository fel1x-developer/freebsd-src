 ...
 * |tanh(x)/x - t(x)| < 2**-72.3
 */
static const union IEEEl2bits T3u = LD80C(0xaaaaaaaaaaaaaa9f, -2,
    -3.33333333333333333017e-1L);
#define T3 T3u.e
static const double T5 = 1.3333333333333314e-1, /*  0x1111111111110a.0p-55 */
 ...
 */
static const long double T3 = -3.33333333333333333333333333333332980e-1L, /* -0x1555555555555555555555555554e.0p-114L
									   */
    ...
    T15 = -1.45583438705131796512568010348874662e-3L; /* -0x17da36452b75e150c44cc34253b34.0p-122L
						       */
static const double T17 = 5.9002744094556621e-4, /*  0x1355824803668e.0p-63 */
...
divl(long double a, long double b, long double c, long double d, long double e,
    long double f)
{
	long double inv, r;
	float fr, fw;
...
tanhl(long double x)
{
	...
	if (ix < 0x4004 || fabsl(x) < 40) { /* |x|<40 */
		if (__predict_false(
			ix < BIAS - (LDBL_MANT_DIG + 1) / 2)) { /* |x|<TINY */
...
#if LDBL_MANT_DIG == 64
			x4 = x2 * x2;
			RETURNI(((T19 * x2 + T17) * x4 + (T15 * x2 + T13)) *
				(x2 * x * x2 * x4 * x4) +
			    ((T11 * x2 + T9) * x4 + (T7 * x2 + T5)) *
				(x2 * x * x2) +
			    T3 * (x2 * x) + x);
#elif LDBL_MANT_DIG == 113
			dx2 = x2;
