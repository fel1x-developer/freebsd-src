...
char copyright[] = "Copyright (c) 1984 through 1996, William LeFebvre";
...
static const struct option longopts[] = { { "cpu-display-mode", no_argument,
					      NULL,
	...
	{ "all", no_argument, NULL, 'a' }, { "batch", no_argument, NULL, 'b' },
	/* c reserved */
	...
	{ "system-idle-procs", no_argument, NULL, 'z' }, { NULL, 0, NULL, 0 } };
...
main(int argc, const char *argv[])
{
	...
	do {
		...
		while ((i = getopt_long(ac, __DECONST(char *const *, av),
			...
			case 'm': /* select display mode */
				if (strcmp(optarg, "io") == 0) {
					...
					errx(1,
					    "warning: `-m' option can only take args 'io' or 'cpu'");
				}
				break;
		...
		/* tricky:  remember old value of preset_argc & set preset_argc
		 * = 0 */
		i = preset_argc;
		preset_argc = 0;
	...
	if (topn == Infinity) {
		 ...
		 * means every process in the system! We only really want to do
		 * that if it was explicitly specified. This is always the case
		 * when "Default_TOPN != Infinity".  But if topn wasn't
		 * explicitly specified and we are on a dumb terminal and the
		 * default is Infinity, then (and only then) we use
...
restart:
	...
	while ((displays == -1) || (displays-- > 0)) {
		...
		processes = get_process_info(&system_info, &ps, compare);
		...
		if (displays) {
			...
			if (!interactive) {
				...
				while (no_command) {
					...
					/* wait for either input or the end of
					 * the delay period */
					sel_ret = select(2, &readfds, NULL,
					    NULL, &timeout);
					...
					if (sel_ret > 0) {
						...
						/* something to read -- clear
						 * the message area first */
						clear_message();

						/* now read it and convert to
						 * command strchr */
						/* (use "change" as a temporary
						 * to hold strchr) */
						if (read(0, &ch, 1) != 1) {
							/* read error: either 0
							 * or -1 */
						...
						if (!no_command) {
							switch (cptr->id) {
							case CMD_redraw: /* redraw
									    screen
									  */
								reset_display();
								break;

							case CMD_update: /* merely
									    update
									    display
									  */
								break;
							...
							case CMD_NONE:
								assert(false &&
								    "reached switch without command");
							}
						}
					}

					/* flush out stuff that may have been
					 * written */
					fflush(stdout);
				}
		}
