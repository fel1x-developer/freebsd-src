...
const struct command all_commands[] = {
	...
	{ '?', NULL, true, CMD_help },
	{ '/', "filter on command name (+ selects all commands)", false,
	    CMD_grep },
...
show_help(void)
{
	...
	while (curcmd->c != 0) {
		...
		if (nextcmd->desc == NULL && nextcmd->c != '\0') {
			...
			/* special case space rather than introducing a "display
			 * string" to the struct */
			sprintf(keys, "SPC");
		} else {
...
err_string(void)
{
	...
	while (cnt < errcnt) {
		...
		if (errp->errnum != currerr) {
			if (currerr >= 0) {
				if ((stringlen = str_adderr(string, stringlen,
					 currerr)) < 2) {
					return (err_listem);
				}
		...
		if ((stringlen = str_addarg(string, stringlen, errp->arg,
			 first)) == 0) {
			return (err_listem);
		}
		first = false;
...
show_errors(void)
{
	...
	while (cnt++ < errcnt) {
		printf("%5s: %s\n", errp->arg,
		    errp->errnum == 0 ? "Not a number" :
					strerror(errp->errnum));
		errp++;
	}
}
