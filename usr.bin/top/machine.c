...
static const char *state_abbrev[] = { "", "START", "RUN\0\0\0", "SLEEP", "STOP",
	"ZOMB", "WAIT", "LOCK" };
...
static const char *procstatenames[] = { "", " starting, ", " running, ",
	" sleeping, ", " stopped, ", " zombie, ", " waiting, ", " lock, ",
	NULL };
...
static const char *cpustatenames[] = { "user", "nice", "system", "interrupt",
	"idle", NULL };

/* these are for detailing the memory statistics */

static const char *memorynames[] = { "K Active, ", "K Inact, ", "K Laundry, ",
	"K Wired, ", "K Buf, ", "K Free", NULL };
static int memory_stats[nitems(memorynames)];

static const char *arcnames[] = { "K Total, ", "K MFU, ", "K MRU, ", "K Anon, ",
	"K Header, ", "K Other", NULL };
static int arc_stats[nitems(arcnames)];

static const char *carcnames[] = { "K Compressed, ", "K Uncompressed, ",
	":1 Ratio, ", NULL };
static int carc_stats[nitems(carcnames)];

static const char *swapnames[] = { "K Total, ", "K Used, ", "K Free, ",
	"% Inuse, ", "K In, ", "K Out", NULL };
 ...
 */
static const char *ordernames[] = { "cpu", "size", "res", "time", "pri",
	"threads", "total", "read", "write", "fault", "vcsw", "ivcsw", "jid",
	"swap", "pid", NULL };
...
machine_init(struct statics *statics)
{
	...
	if (sysctlbyname("kstat.zfs.misc.arcstats.size", &arc_size, &size, NULL,
		0) == 0 &&
	    arc_size != 0)
		arc_enabled = 1;
	size = sizeof(carc_en);
...
format_header(const char *uname_field)
{
	...
	case DISP_IO: {
		sbuf_printf(header, "  %s%*s %-*.*s",
		    ps.thread_id ? " THR" : "PID", ps.jail ? TOP_JID_LEN : 0,
		    ps.jail ? " JID" : "", namelength, namelength, uname_field);
		sbuf_cat(header,
...
get_system_info(struct system_info *si)
{
	...
	for (i = j = 0; i <= maxid; i++) {
		...
		percentages(CPUSTATES, &pcpu_cpu_states[j * CPUSTATES],
		    &pcpu_cp_time[j * CPUSTATES], &pcpu_cp_old[j * CPUSTATES],
    ...
    long *vcsw, long *ivcsw)
{
	...
	*ivcsw = RU(pp)->ru_nivcsw - RU(oldp)->ru_nivcsw;
	ret = (RU(pp)->ru_inblock - RU(oldp)->ru_inblock) +
    ...
    int (*compare)(const void *, const void *))
{
	...
	for (pp = pbase, i = 0; i < nproc; pp++, i++) {
		...
		if (displaymode == DISP_CPU && !sel->idle &&
		    (!proc_used_cpu(pp) || pp->ki_stat == SSTOP ||
			pp->ki_stat == SIDL))
...
format_next_process(struct handle *xhandle, char *(*get_userid)(int), int flags)
{
	...
	if (!(flags & FMT_SHOWARGS)) {
		...
		if (pp->ki_flag & P_SYSTEM ||
			...
			if (ps.thread && pp->ki_flag & P_HADTHREADS &&
			    pp->ki_tdname[0]) {
				snprintf(cmdbuf, screen_width, "[%s{%s%s}]",
				    pp->ki_comm, pp->ki_tdname,
				    pp->ki_moretdname);
			} else {
				snprintf(cmdbuf, screen_width, "[%s]",
				    pp->ki_comm);
			}
		} else {
			const char *src;
	...
	if (displaymode == DISP_IO) {
		...
		sbuf_printf(procbuf, "%5d ",
		    (ps.thread_id) ? pp->ki_tid : pp->ki_pid);

		if (ps.jail) {
			sbuf_printf(procbuf, "%*d ", TOP_JID_LEN - 1,
			    pp->ki_jid);
		}
		...
		if (ps.swap) {
			sbuf_printf(procbuf, "%*s ", TOP_SWAP_LEN - 1,
		...
		sbuf_printf(procbuf, "%6.2f%% ",
		    ps.wcpu ? 100.0 * weighted_cpu(PCTCPU(pp), pp) :
			      100.0 * PCTCPU(pp));
	}
...
format_nice(const struct kinfo_proc *pp)
{
	...
	case PRI_REALTIME:
		...
		rtpri = ((pp->ki_flag & P_KPROC) ? pp->ki_pri.pri_native :
						   pp->ki_pri.pri_user) -
		    PRI_MIN_REALTIME;
		snprintf(nicebuf, sizeof(nicebuf), "%sr%d%s", kproc, rtpri,
		    fifo);
		break;
	case PRI_TIMESHARE:
	...
	case PRI_IDLE:
		...
		rtpri = ((pp->ki_flag & P_KPROC) ? pp->ki_pri.pri_native :
						   pp->ki_pri.pri_user) -
		    PRI_MIN_IDLE;
		snprintf(nicebuf, sizeof(nicebuf), "%si%d%s", kproc, rtpri,
		    fifo);
		break;
	default:
		return ("?");
...
#define ORDERKEY_CPTICKS(a, b)                            \
	do {                                              \
		int64_t diff = (int64_t)(b)->ki_runtime - \
		    (int64_t)(a)->ki_runtime;             \
		if (diff != 0)                            \
			return (diff > 0 ? 1 : -1);       \
	} while (0)

#define ORDERKEY_STATE(a, b)                                           \
	do {                                                           \
...
#define ORDERKEY_PRIO(a, b)                             \
	do {                                            \
		int diff = (int)(b)->ki_pri.pri_level - \
		    (int)(a)->ki_pri.pri_level;         \
		if (diff != 0)                          \
			return (diff > 0 ? 1 : -1);     \
	} while (0)

#define ORDERKEY_THREADS(a, b)                                                \
	do {                                                                  \
...
{
	const struct kinfo_proc *const p1 = *(
	    const struct kinfo_proc *const *)arg1;
	const struct kinfo_proc *const p2 = *(
	    const struct kinfo_proc *const *)arg2;
...
int (*compares[])(const void *arg1, const void *arg2) = { compare_cpu,
	compare_size, compare_res, compare_time, compare_prio, compare_threads,
	compare_iototal, compare_ioread, compare_iowrite, compare_iofault,
	compare_vcsw, compare_ivcsw, compare_jid, compare_swap, compare_pid,
	NULL };
