	...
	while ((option = getopt(argc, argv, "ef:r:sSt:T:U:vV")) != -1) {
		...
		case 'r':
			...
			if (report_fp != NULL) {
				...
				fatal(2, "cannot open \"%s\" for appending",
				    report_file);
			break;
...
terminate(int status)
{
	...
	if (report_file != (char *)0 && report_fp != (FILE *)NULL) {
		/*
		 * Allow the last of the report string to be gathered before we
		 * terminate.
		 */
		if (report_gathering) {
			int c;
	...
	while (*s && s1 - temp < (off_t)(sizeof(temp) - 2 - add_return)) {
		...
		case 'U':
			if (sending && phone_num2) {
				for (phchar = phone_num2; *phchar != '\0';
				     phchar++)
					*s1++ = *phchar;
			} else {
				*s1++ = '\\';
...
chat_expect(char *s)
{
	...
	for (;;) {
		...
		/*
		 * If there is a sub-reply string then send it. Otherwise any
...
chat_send(char *s)
{
	...
	if (clear_abort_next) {
		...
		if (strlen(s1) > strlen(s) ||
		    strlen(s1) + 1 > sizeof(fail_buffer))
			fatal(1, "Illegal or too-long CLR_ABORT string ('%v')",
			    s);
	...
	if (report_next) {
		...
		if (strlen(s1) > strlen(s) ||
		    strlen(s1) > sizeof fail_buffer - 1)
...
put_string(char *s)
{
	...
	while (*s) {
		...
		case 'p':
			usleep(10000); /* 1/100th of a second (arg is
					  microseconds) */
			break;
...
get_string(char *string)
{
	...
	while (!alarmed && (c = get_char()) >= 0) {
		...
		*s++ = c;
			...
			for (n = 0; n < n_reports; ++n) {
				if ((report_string[n] != (char *)NULL) &&
				    s - temp >= (report_len = strlen(
						     report_string[n])) &&
				    strncmp(s - report_len, report_string[n],
					report_len) == 0) {
					time_t time_now = time((time_t *)NULL);
					struct tm *tm_now = localtime(
					    &time_now);

					strftime(report_buffer, 20,
					    "%b %d %H:%M:%S ", tm_now);
		...
		if ((size_t)(s - temp) >= len && c == string[len - 1] &&
		...
		for (n = 0; n < n_aborts; ++n) {
			if (s - temp >= (abort_len = strlen(abort_string[n])) &&
			    strncmp(s - abort_len, abort_string[n],
				abort_len) == 0) {
				if (verbose) {
					if (s > logged)
						chat_logf("%0.*v", s - logged,
						    logged);
					chat_logf(" -- failed");
				}
				...
				alarmed = 0;
				exit_code = n + 4;
				strcpy(fail_reason = fail_buffer,
				    abort_string[n]);
				return (0);
			}
		}
