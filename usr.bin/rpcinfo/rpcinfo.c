...
#ifdef PORTMAP
static CLIENT *
clnt_com_create(struct sockaddr_in *addr, u_long prog, u_long vers, int *fdp,
    const char *trans)
{
	CLIENT *clnt;
...
pmapdump(int argc, char **argv)
{
	...
	clnt_st = CLNT_CALL(client, PMAPPROC_DUMP, (xdrproc_t)xdr_void, NULL,
	    (xdrproc_t)xdr_pmaplist_ptr, (char *)&head, minutetimeout);
	...
	if (head == NULL) {
		...
		for (; head != NULL; head = head->pml_next) {
			printf("%10ld%5ld", head->pml_map.pm_prog,
...
get_inet_address(struct sockaddr_in *addr, char *host)
{
	...
	if (addr->sin_addr.s_addr == INADDR_NONE ||
		...
		if ((nconf = __rpc_getconfip("udp")) == NULL &&
			...
			if ((error = getaddrinfo(host, "rpcbind", &hints,
				 &res)) != 0)
...
brdcst(int argc, char **argv)
{
	...
	vers = getvers(argv[1]);
	rpc_stat = rpc_broadcast(prognum, vers, NULLPROC, (xdrproc_t)xdr_void,
...
rpcbdump(int dumptype, char *netid, int argc, char **argv)
{
	...
	minutetimeout.tv_sec = 60;
	minutetimeout.tv_usec = 0;
	clnt_st = CLNT_CALL(client, RPCBPROC_DUMP, (xdrproc_t)xdr_void, NULL,
	    (xdrproc_t)xdr_rpcblist_ptr, (char *)&head, minutetimeout);
	if (clnt_st != RPC_SUCCESS) {
		if ((clnt_st == RPC_PROGVERSMISMATCH) ||
		    (clnt_st == RPC_PROGUNAVAIL)) {
			int vers;
			...
			if (err.re_vers.low == RPCBVERS4) {
				...
				if (err.re_vers.high == PMAPVERS) {
					...
					clnt_st = CLNT_CALL(client,
					    PMAPPROC_DUMP, (xdrproc_t)xdr_void,
					...
					for (head = NULL; pmaphead != NULL;
						...
						list->rpcb_next = NULL;
						list->rpcb_map.r_prog =
						    pmaphead->pml_map.pm_prog;
						list->rpcb_map.r_vers =
						    pmaphead->pml_map.pm_vers;
						if (pmaphead->pml_map.pm_prot ==
						    IPPROTO_UDP)
							list->rpcb_map.r_netid =
							    "udp";
						else if (pmaphead->pml_map
							     .pm_prot ==
						    IPPROTO_TCP)
							list->rpcb_map.r_netid =
							    "tcp";
...
#define MAXLONG_AS_STRING "2147483648"
							...
							if (list->rpcb_map
							    ...
							    NULL)
								goto error;
							sprintf(list->rpcb_map
								    .r_netid,
							    "%6ld",
							    pmaphead->pml_map
								.pm_prot);
						}
						list->rpcb_map.r_owner =
						    UNKNOWN;
						low =
						    pmaphead->pml_map.pm_port &
						    0xff;
						high = (pmaphead->pml_map
							       .pm_port >>
							   8) &
						    0xff;
						list->rpcb_map.r_addr = strdup(
						    "0.0.0.0.XXX.XXX");
						sprintf(
						    &list->rpcb_map.r_addr[8],
						    "%d.%d", high, low);
						prev = list;
					}
				}
	...
	if (head == NULL) {
		...
		for (; head != NULL; head = head->rpcb_next) {
			printf("%10u%5u    ", head->rpcb_map.r_prog,
			    head->rpcb_map.r_vers);
		...
		for (; head != NULL; head = head->rpcb_next) {
			for (rs = rs_head; rs; rs = rs->next)
				if (head->rpcb_map.r_prog == rs->prog)
					break;
			if (rs == NULL) {
				rs = (struct rpcbdump_short *)malloc(
				    sizeof(struct rpcbdump_short));
				if (rs == NULL)
					goto error;
				rs->next = NULL;
				if (rs_head == NULL) {
...
rpcbaddrlist(char *netid, int argc, char **argv)
{
	...
	if (head == NULL) {
		...
		for (; head != NULL; head = head->rpcb_entry_next) {
			...
			sprintf(buf, "%s/%s/%s ", re->r_nc_protofmly,
			    re->r_nc_proto,
...
#define MAXLINE 256
	...
	char *cp, *lp;
	const char *pmaphdr[] = { "NULL", "SET", "UNSET", "GETPORT", "DUMP",
		"CALLIT" };
	const char *rpcb3hdr[] = { "NULL", "SET", "UNSET", "GETADDR", "DUMP",
		"CALLIT", "TIME", "U2T", "T2U" };
	const char *rpcb4hdr[] = { "NULL", "SET", "UNSET", "GETADDR", "DUMP",
		"CALLIT", "TIME", "U2T", "T2U", "VERADDR", "INDRECT", "GETLIST",
		"GETSTAT" };
...
#define TABSTOP 8
	...
	for (i = 0; i <= rpcb_highproc_2; i++) {
		...
		printf("%s%s", pmaphdr[i],
		    spaces(
			(TABSTOP * (1 + flen / TABSTOP)) - strlen(pmaphdr[i])));
 ...
 */
static CLIENT *
clnt_addr_create(char *address, struct netconfig *nconf, u_long prog,
    u_long vers)
{
	CLIENT *client;
...
{
	static const char *tlist[3] = { "circuit_n", "circuit_v",
		"datagram_v" };
	int i;
	struct netconfig *nconf;
	CLIENT *clnt = NULL;
...
static CLIENT *
getclnthandle(char *host, struct netconfig *nconf, u_long rpcbversnum,
    struct netbuf **targaddr)
{
	struct netbuf addr;
	struct addrinfo hints, *res;
	...
	client = clnt_tli_create(RPC_ANYFD, nconf, &addr, RPCBPROG, rpcbversnum,
	    0, 0);
	if (client) {
		if (targaddr != NULL) {
			*targaddr = (struct netbuf *)malloc(
