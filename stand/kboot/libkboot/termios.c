 ...
 *
 * Note: From the musl project, stripped down and repackaged with HOST_/host_
 * prepended
 */
...
host_cfmakeraw(struct host_termios *t)
{
	...
	t->c_lflag &= ~(
	    HOST_ECHO | HOST_ECHONL | HOST_ICANON | HOST_ISIG | HOST_IEXTEN);
...
int
host_cfsetospeed(struct host_termios *tio, host_speed_t speed)
{
	if (speed & ~HOST_CBAUD) {
		//		errno = EINVAL; /* XXX ? */
		return -1;
...
int
host_cfsetispeed(struct host_termios *tio, host_speed_t speed)
{
