...
/* Same system call with different names on different Linux architectures due to
 * history */
int
...
ssize_t
host_llseek(int fd, int32_t offset_high, int32_t offset_lo, uint64_t *result,
    int whence)
{
#ifdef SYS_llseek
	return host_syscall(SYS_llseek, fd, offset_high, offset_lo,
	    (uintptr_t)result, whence);
#else
	int64_t rv = host_syscall(SYS_lseek, fd,
	    (int64_t)((uint64_t)offset_high << 32 | (uint32_t)offset_lo),
	    whence);
	if (rv > 0)
		*result = (uint64_t)rv;
	return (rv);
...
int
host_kexec_load(unsigned long entry, unsigned long nsegs,
...
{
	return (void *)host_syscall(SYS_mmap, (uintptr_t)addr, len, prot, flags,
	    fd, off);
}

int
host_mount(const char *src, const char *target, const char *type,
...
{
	return host_syscall(SYS_openat, HOST_AT_FDCWD, (uintptr_t)path, flags,
	    mode);
    ...
    struct host_timeval *timeout)
{
	 ...
	 * pass a NULL.
	 */
	return host_syscall(SYS_pselect6, nfds, (uintptr_t)readfds,
	    (uintptr_t)writefds, (uintptr_t)exceptfds, (uintptr_t)&ts,
	    (uintptr_t)NULL);
}
