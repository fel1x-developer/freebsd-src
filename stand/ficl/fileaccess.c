...
static void
pushIor(FICL_VM *pVM, int success)
{
	int ior;
	if (success)
		ior = 0;
...
static void
ficlFopen(FICL_VM *pVM, char *writeMode) /* ( c-addr u fam -- fileid ior ) */
{
...
static void
ficlOpenFile(FICL_VM *pVM) /* ( c-addr u fam -- fileid ior ) */
{
	ficlFopen(pVM, "a");
}

static void
ficlCreateFile(FICL_VM *pVM) /* ( c-addr u fam -- fileid ior ) */
{
	ficlFopen(pVM, "w");
}

static int
closeFiclFILE(ficlFILE *ff) /* ( fileid -- ior ) */
{
	FILE *f = ff->f;
	free(ff);
	return !fclose(f);
...
static void
ficlCloseFile(FICL_VM *pVM) /* ( fileid -- ior ) */
{
...
static void
ficlDeleteFile(FICL_VM *pVM) /* ( c-addr u -- ior ) */
{
	int length = stackPopINT(pVM->pStack);
	void *address = (void *)stackPopPtr(pVM->pStack);
...
static void
ficlRenameFile(FICL_VM *pVM) /* ( c-addr1 u1 c-addr2 u2 -- ior ) */
{
	int length;
	void *address;
	char *from;
...
static void
ficlFileStatus(FICL_VM *pVM) /* ( c-addr u -- x ior ) */
{
	struct stat statbuf;
	...
	if (stat(filename, &statbuf) == 0) {
		...
		** I push the file's access mode (readable, writeable, is
		*directory, etc)
...
static void
ficlFilePosition(FICL_VM *pVM) /* ( fileid -- ud ior ) */
{
	ficlFILE *ff = (ficlFILE *)stackPopPtr(pVM->pStack);
	long ud = ftell(ff->f);
...
static void
ficlFileSize(FICL_VM *pVM) /* ( fileid -- ud ior ) */
{
	ficlFILE *ff = (ficlFILE *)stackPopPtr(pVM->pStack);
...
#define nLINEBUF 256
static void
ficlIncludeFile(FICL_VM *pVM) /* ( i*x fileid -- j*x ) */
{
	ficlFILE *ff = (ficlFILE *)stackPopPtr(pVM->pStack);
...
static void
ficlReadFile(FICL_VM *pVM) /* ( c-addr u1 fileid -- u2 ior ) */
{
	ficlFILE *ff = (ficlFILE *)stackPopPtr(pVM->pStack);
	int length = stackPopINT(pVM->pStack);
	void *address = (void *)stackPopPtr(pVM->pStack);
...
static void
ficlReadLine(FICL_VM *pVM) /* ( c-addr u1 fileid -- u2 flag ior ) */
{
	ficlFILE *ff = (ficlFILE *)stackPopPtr(pVM->pStack);
	int length = stackPopINT(pVM->pStack);
...
static void
ficlWriteFile(FICL_VM *pVM) /* ( c-addr u1 fileid -- ior ) */
{
	ficlFILE *ff = (ficlFILE *)stackPopPtr(pVM->pStack);
	int length = stackPopINT(pVM->pStack);
	void *address = (void *)stackPopPtr(pVM->pStack);
...
static void
ficlWriteLine(FICL_VM *pVM) /* ( c-addr u1 fileid -- ior ) */
{
	ficlFILE *ff = (ficlFILE *)stackPopPtr(pVM->pStack);
...
static void
ficlRepositionFile(FICL_VM *pVM) /* ( ud fileid -- ior ) */
{
	ficlFILE *ff = (ficlFILE *)stackPopPtr(pVM->pStack);
...
static void
ficlFlushFile(FICL_VM *pVM) /* ( fileid -- ior ) */
{
	ficlFILE *ff = (ficlFILE *)stackPopPtr(pVM->pStack);
...
static void
ficlResizeFile(FICL_VM *pVM) /* ( ud fileid -- ior ) */
{
	ficlFILE *ff = (ficlFILE *)stackPopPtr(pVM->pStack);
	size_t ud = (size_t)stackPopINT(pVM->pStack);
...
void
ficlCompileFile(FICL_SYSTEM *pSys)
{
#if FICL_WANT_FILE
