...
main(int argc, char **argv)
{
	...
	if (!broadcast) {
		...
		if (clnt == NULL)
			errx(1, "could not contact bootparam server on host %s",
			    server);
	}
	...
	case 3:
		...
		bcopy(&the_inet_addr,
		    &whoami_arg.client_address.bp_address_u.ip_addr, 4);

		if (!broadcast) {
			whoami_res = bootparamproc_whoami_1(&whoami_arg, clnt);
			printf("Whoami returning:\n");
			if (printwhoami(whoami_res)) {
				errx(1, "bad answer returned from server %s",
				    server);
			} else
				exit(0);
		} else {
			(void)clnt_broadcast(BOOTPARAMPROG, BOOTPARAMVERS,
			    BOOTPARAMPROC_WHOAMI, (xdrproc_t)xdr_bp_whoami_arg,
			    (char *)&whoami_arg, (xdrproc_t)xdr_bp_whoami_res,
	...
	case 4:
		...
		if (!broadcast) {
			getfile_res = bootparamproc_getfile_1(&getfile_arg,
			    clnt);
			...
			(void)clnt_broadcast(BOOTPARAMPROG, BOOTPARAMVERS,
			    ...
			    (xdrproc_t)xdr_bp_getfile_arg, (char *)&getfile_arg,
...
printgetfile(bp_getfile_res *res)
{
	if (res) {
		printf("server_name:\t%s\nserver_address:\t%s\npath:\t%s\n",
		    res->server_name,
		    inet_ntoa(*(struct in_addr *)&res->server_address
				   .bp_address_u.ip_addr),
		    res->server_path);
		return (0);
	} else {
		warnx("null answer!!!");
