	...
	if (checkhost(askname, hostname, sizeof hostname)) {
		...
		if (debug)
			fprintf(stderr, "Returning %s   %s    %d.%d.%d.%d\n",
			    ...
			    255 &
				res.router_address.bp_address_u.ip_addr.impno);
		if (dolog)
...
bp_getfile_res *
bootparamproc_getfile_1_svc(bp_getfile_arg *getfile,
    struct svc_req *req __unused)
{
	...
	if (getthefile(askname, getfile->file_id, buffer, sizeof(buffer))) {
		if ((where = strchr(buffer, ':'))) {
			/* buffer is re-written to contain the name of the info
			 * of file */
			...
			if (!he)
				goto failed;
			bcopy(he->h_addr,
			    &res.server_address.bp_address_u.ip_addr, 4);
			...
			if (!strcmp(getfile->file_id, "dump")) {
				...
				res.server_path = "";
				res.server_address.address_type = IP_ADDR_TYPE;
				bzero(&res.server_address.bp_address_u.ip_addr,
				    4);
			} else
				goto failed;
		}
		if (debug)
			fprintf(stderr,
			    ...
			    255 &
				res.server_address.bp_address_u.ip_addr.impno);
		if (dolog)
			syslog(LOG_NOTICE,
			    "returning server:%s path:%s address: %d.%d.%d.%d\n",
			    res.server_name, res.server_path,
...
failed:
	...
	if (dolog)
		syslog(LOG_NOTICE, "getfile failed for %s\n",
		    getfile->client_name);
	return (NULL);
}
...
getthefile(char *l_askname, char *fileid, char *buf, int blen __unused)
{
	...
	int match = 0;
#define INFOLEN 1343
	_Static_assert(INFOLEN >=
		MAX_FILEID + MAX_PATH_LEN + MAX_MACHINE_NAME + 3,
	...
	while (fscanf(bpf, "%255s", hostname) > 0 && !match) {
		if (*hostname != '#') { /* comment */
			if (!strcmp(hostname, l_askname)) {
				...
				he = gethostbyname(hostname);
				if (he && !strcmp(he->h_name, l_askname))
					match = 1;
			}
		}
		if (*hostname == '+') { /* NIS */
			...
			if (yp_match(yp_domain, "bootparams", l_askname,
				strlen(l_askname), &result, &resultlen))
				return (0);
			if (strstr(result, fileid) == NULL) {
				buf[0] = '\0';
			} else {
				snprintf(buf, blen, "%s",
				    strchr(strstr(result, fileid), '=') + 1);
				if (strchr(buf, ' ') != NULL)
					*(char *)(strchr(buf, ' ')) = '\0';
			}
			if (fclose(bpf))
			...
			if (*info != '#') { /* Comment ? */
				if (!strncmp(info, fileid, fid_len) &&
				    *(info + fid_len) == '=') {
					...
					if (ch == '\n') { /* didn't find it */
						res = -1;
						break; /* but host is there */
					}
					if (ch == '\\') {	/* more info */
						ch = getc(bpf); /* maybe on next
								   line */
						if (ch == '\n')
							continue; /* read it in
								     next loop
						...
						ungetc(ch,
						    bpf); /* but who know what a
							     `\` is */
				}			  /* needed for. */
			} else
				break; /* a commented rest-of-line */
		}
	}
	if (fclose(bpf)) {
		warnx("could not close %s", bootpfile);
