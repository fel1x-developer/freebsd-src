...
static struct cmdtab {
	...
	unsigned min;
	const char *args;
} cmdtab[] = { { CMD_CLOSE, "close", 1, "" },
	{ CMD_DEBUG, "debug", 1, "on | off" }, { CMD_EJECT, "eject", 1, "" },
	{ CMD_HELP, "?", 1, 0 }, { CMD_HELP, "help", 1, "" },
	{ CMD_INFO, "info", 1, "" }, { CMD_NEXT, "next", 1, "" },
	{ CMD_PAUSE, "pause", 2, "" },
	...
	{ CMD_PREVIOUS, "previous", 2, "" }, { CMD_QUIT, "quit", 1, "" },
	{ CMD_RESET, "reset", 4, "" }, { CMD_RESUME, "resume", 1, "" },
	{ CMD_SET, "set", 2, "msf | lba" },
	{ CMD_STATUS, "status", 1, "[audio | media | volume]" },
	{ CMD_STOP, "stop", 3, "" },
	{ CMD_VOLUME, "volume", 1,
	    "<l&r> <l> <r> | left | right | mute | mono | stereo" },
	{ CMD_CDID, "cdid", 2, "" }, { CMD_SPEED, "speed", 2, "speed" },
	{ 0, NULL, 0, NULL } };
...
use_cdrom_instead(const char *old_envvar)
{
	...
	if (device)
		warnx("%s environment variable deprecated, "
		      "please use CDROM in the future.",
		    old_envvar);
	return device;
}
...
play(char *arg)
{
	...
	if (strchr(arg, ':')) {
		...
		tr2 = m2 = s2 = f2 = f1 = 0;
		if (8 ==
		    sscanf(arg, "%d %d:%d.%d %d %d:%d.%d", &tr1, &m1, &s1, &f1,
			&tr2, &m2, &s2, &f2))
			goto Play_Relative_Addresses;

		tr2 = m2 = s2 = f2 = f1 = 0;
		if (7 ==
		    sscanf(arg, "%d %d:%d %d %d:%d.%d", &tr1, &m1, &s1, &tr2,
			&m2, &s2, &f2))
			goto Play_Relative_Addresses;

		tr2 = m2 = s2 = f2 = f1 = 0;
		if (7 ==
		    sscanf(arg, "%d %d:%d.%d %d %d:%d", &tr1, &m1, &s1, &f1,
			&tr2, &m2, &s2))
			goto Play_Relative_Addresses;
		...
		tr2 = m2 = s2 = f2 = f1 = 0;
		if (6 ==
		    sscanf(arg, "%d %d:%d.%d %d:%d", &tr1, &m1, &s1, &f1, &m2,
			&s2))
			goto Play_Relative_Addresses;

		tr2 = m2 = s2 = f2 = f1 = 0;
		if (6 ==
		    sscanf(arg, "%d %d:%d %d:%d.%d", &tr1, &m1, &s1, &m2, &s2,
			&f2))
			goto Play_Relative_Addresses;

		tr2 = m2 = s2 = f2 = f1 = 0;
		if (6 ==
		    sscanf(arg, "%d %d:%d.%d %d %d", &tr1, &m1, &s1, &f1, &tr2,
			&m2))
			goto Play_Relative_Addresses;

		tr2 = m2 = s2 = f2 = f1 = 0;
		if (5 ==
		    sscanf(arg, "%d %d:%d %d:%d", &tr1, &m1, &s1, &m2, &s2))
			goto Play_Relative_Addresses;

		tr2 = m2 = s2 = f2 = f1 = 0;
		if (5 ==
		    sscanf(arg, "%d %d:%d %d %d", &tr1, &m1, &s1, &tr2, &m2))
			goto Play_Relative_Addresses;

		tr2 = m2 = s2 = f2 = f1 = 0;
		if (5 ==
		    sscanf(arg, "%d %d:%d.%d %d", &tr1, &m1, &s1, &f1, &tr2))
			goto Play_Relative_Addresses;
	...
	Play_Relative_Addresses:
		...
		if (!tr2) {
			...
			if (msf) {
				...
				lba2msf(ntohl(toc_buffer[tr2].addr.lba), &tm,
				    &ts, &tf);
			f2 += tf;
			if (f2 >= 75) {
				s2 += f2 / 75;
		...
		if ((tr2 < n) &&
			...
			((m2 == tm) &&
			    ((s2 > ts) || ((s2 == ts) && (f2 > tf)))))) {
			printf(
			    "The playing time of the disc is not that long.\n");
			return (0);
		}
	...
	Try_Absolute_Timed_Addresses:
		if (6 !=
			sscanf(arg, "%d:%d.%d%d:%d.%d", &m1, &s1, &f1, &m2, &s2,
			    &f2) &&
		    5 !=
			sscanf(arg, "%d:%d.%d%d:%d", &m1, &s1, &f1, &m2, &s2) &&
		    5 !=
			sscanf(arg, "%d:%d%d:%d.%d", &m1, &s1, &m2, &s2, &f2) &&
		...
		if (m2 == 0) {
			if (msf) {
				...
				lba2msf(ntohl(toc_buffer[n].addr.lba), &tm, &ts,
				    &tf);
				m2 = tm;
				s2 = ts;
				f2 = tf;
	...
	if (what & STATUS_MEDIA) {
		...
		if (rc >= 0) {
			...
			if (ss.data->what.media_catalog.mc_valid &&
				...
				printf(", number \"%.*s\"", (int)sizeof(vmcn),
				    vmcn);
			}
			putchar('\n');
		} else
	...
	*trk = s.data->what.position.track_number;
	if (msf) {
		...
		*frame = s.data->what.position.reladdr.msf.frame;
		...
		lba2msf(ntohl(s.data->what.position.reladdr.lba), &mm, &ss,
		    &ff);
		*min = mm;
		*sec = ss;
		*frame = ff;
