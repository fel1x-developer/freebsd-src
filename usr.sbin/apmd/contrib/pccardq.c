...
connect_to_pccardd(char **path)
{
	...
	snprintf(pccardq.sun_path, sizeof pccardq.sun_path, "%s/%s%ld%ld",
	    tmp_dir, prog, (long)getpid(), (long)time(0));
	pccardq.sun_family = AF_UNIX;
	pccardq.sun_len = offsetof(struct sockaddr_un, sun_path) +
	    strlen(pccardq.sun_path);
...
enum { SLOT_EMPTY = 0, SLOT_FILLED = 1, SLOT_INACTIVE = 2, SLOT_UNDEFINED = 9 };

int
get_slot_info(int so, int slot, char **manuf, char **version, char **device,
    int *state)
{
	int rc = -1;
	int rv;
	...
	if (slot_map == 0) {
		...
		for (i = 0; i < nslot; i++) {
			if ((slot_map & (1 << i))) {
				...
				if (get_slot_info(so, i, &manuf, &version,
					&device, &state) < 0)
					goto out;
				if (manuf == NULL || version == NULL ||
				    device == NULL)
					goto out;
				printf("%d~%s~%s~%s~%s\n", i, manuf, version,
				    device, strstate(state));
				free(manuf);
				free(version);
				free(device);
			}
