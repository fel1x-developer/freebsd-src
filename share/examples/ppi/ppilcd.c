...
static struct lcd_driver lcd_drivertab[] = {
	{ "hd44780", "Hitachi HD44780 and compatibles",
	    { "Reset options:", "    1     1-line display (default 2)",
		"    B     Cursor blink enable", "    C     Cursor enable",
		"    F     Large font select", NULL },
	    hd44780_prepare, hd44780_finish, hd44780_command, hd44780_putc },
...
main(int argc, char *argv[])
{
	...
	if (argc > 0) {
		debug(2, "reading input from %d argument%s", argc,
		    (argc > 1) ? "s" : "");
		for (i = 0; i < argc; i++)
			for (cp = argv[i]; *cp; cp++)
				do_char(driver, *cp);
...
usage(void)
{
	...
	for (i = 0; lcd_drivertab[i].l_code != NULL; i++) {
		fprintf(stderr, "              %-10s (%s)%s\n",
		    lcd_drivertab[i].l_code, lcd_drivertab[i].l_name,
		    (i == 0) ? " *default*" : "");
...
#define hd_sctrl(v)                            \
	{                                      \
		u_int8_t _val;                 \
		_val = hd_cbits | v;           \
		ioctl(hd_fd, PPISCTRL, &_val); \
	}
#define hd_sdata(v)                            \
	{                                      \
		u_int8_t _val;                 \
		_val = v;                      \
		ioctl(hd_fd, PPISDATA, &_val); \
	}
...
hd44780_prepare(char *devname, char *options)
{
	...
	while (cp && *cp) {
		...
		case 'F':
			...
			break;
		default:
			errx(EX_USAGE, "hd44780: unknown option code '%c'",
			    *(cp - 1));
		}
	}
	...
	hd_cbits &= ~(
	    STROBE | SELECTIN | AUTOFEED); /* set strobe, RS, R/W low */
...
hd44780_command(int cmd)
{
	...
	case CMD_RESET: /* full manual reset and reconfigure as per datasheet */
		debug(1, "hd44780: reset to %d lines, %s font,%s%s cursor",
		    hd_lines, hd_font ? "5x10" : "5x7", hd_cursor ? "" : " no",
		    hd_blink ? " blinking" : "");
		val = 0x30;
		if (hd_lines == 2)
			val |= 0x08;
