...
doit(struct libusb20_device *dev)
{
	...
	if (do_request)
		printf(
		    "doit(): bmRequestType 0x%02x, bRequest 0x%02x, wValue 0x%04x, wIndex 0x%04x, wLength 0x%04x\n",
	...
	if (do_request) {
		if ((rv = libusb20_dev_request_sync(dev, &setup, data, &actlen,
			 TIMEOUT, 0 /* flags */)) != 0) {
			fprintf(stderr, "libusb20_dev_request_sync: %s\n",
			    libusb20_strerror(rv));
		}
		printf("sent %d bytes\n", actlen);
		if ((setup.bmRequestType & 0x80) != 0) {
	...
	if (intr_ep != 0) {
		 ...
		 */
		struct libusb20_transfer *xfr_intr =
		    libusb20_tr_get_pointer(dev, 0);
		...
		if ((rv = libusb20_tr_bulk_intr_sync(xfr_intr, in_buf, BUFLEN,
			 &rlen, TIMEOUT)) != 0) {
...
get_req(const char *reqname)
{
	...
	for (i = 0; i < sizeof reqnames / sizeof reqnames[0]; i++)
	...
	for (idx = 0; argc != 0 && idx <= 6; argc--, idx++)
		...
		case 0:
			...
			if (*argv[idx] == 'i')
				...
				fprintf(stderr,
				    "request direction must be \"in\" or \"out\" (got %s)\n",
				    argv[idx]);
				return -1;
			}
			break;
...
main(int argc, char **argv)
{
	 ...
	 *	                                       (device, interface,
	 *endpoint, other) setup.bRequest:      the request itself (see
	 *get_req() for standard requests, or specific value) setup.wValue: a
	 *16-bit value setup.wIndex:        another 16-bit value setup.wLength:
	 *length of associated data transfer, direction depends on bit 7 of
	 *bmRequestType
	 */
	...
	if (vid != UINT_MAX || pid != UINT_MAX) {
		if (intr_ep != 0 && (intr_ep & 0x80) == 0) {
			fprintf(stderr,
			    "Interrupt endpoint must be of type IN\n");
			usage();
		}
		...
		if (argc > 0) {
			...
			if (argc > 0) {
				for (out_len = 0; argc > 0 && out_len < BUFLEN;
				     out_len++, argc--) {
					unsigned n = strtoul(argv[out_len], 0,
					    0);
					if (n > 255)
						fprintf(stderr,
