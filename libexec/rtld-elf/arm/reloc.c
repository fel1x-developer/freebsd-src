	...
	case R_ARM_PC24: { /* word32 S - P + A */
		...
		*where = (*where & 0xff000000) | (tmp & 0x00ffffff);
		dbg("PC24 %s in %s --> %p @ %p in %s",
		    obj->strtab + obj->symtab[symnum].st_name, obj->path,
	...
	case R_ARM_GLOB_DAT: /* word32 B + S */
		...
		if (__predict_true(RELOC_ALIGNED_P(where))) {
			...
			*where = tmp;
			...
			tmp = load_ptr(where) + (Elf_Addr)defobj->relocbase +
			    def->st_value;
			store_ptr(where, tmp);
		}
		dbg("ABS32/GLOB_DAT %s in %s --> %p @ %p in %s",
		    obj->strtab + obj->symtab[symnum].st_name, obj->path,
		    (void *)tmp, where, defobj->path);
		break;
	...
	case R_ARM_RELATIVE: /* word32 B + A */
		if (__predict_true(RELOC_ALIGNED_P(where))) {
			...
			*where = tmp;
			...
			tmp = load_ptr(where) + (Elf_Addr)obj->relocbase;
			store_ptr(where, tmp);
		}
		dbg("RELATIVE in %s --> %p", obj->path, (void *)tmp);
		break;
...
reloc_jmpslots(Obj_Entry *obj, int flags, RtldLockState *lockstate)
{
	...
	for (rel = obj->pltrel; rel < rellim; rel++) {
		...
		reloc_jmpslot(where, target, defobj, obj, (const Elf_Rel *)rel);
	}
