...
class Handler {
	...
	static void PrintVector(std::string prefix,
	...
	void Usage(std::string msg, bool exit_with_error)
	{
		...
		std::cerr << binary_name
			  << ": See atf-test-program(1) for usage details."
			  << std::endl;
	...
	// Running test:
	// atf_wrap '-P /path' /path/to/script -r /path1 -s /path2 -vk1=v1
	// testname
	void Parse(int argc, char **argv)
	{
		...
		// getopt() skips the first argument (as it is typically binary
		// name) it is possible to have either '-P\s*/path' followed by
		// the script name or just the script name. Parse
		// kernel-provided arg manually and adjust array to make getopt
		// work
		...
		// The next argument is a script name. Copy and keep argc/argv
		// the same Show usage for empty args
		if (argc == 0) {
		...
		while ((c = getopt(argc, argv, "lr:s:v:")) != -1) {
			...
			case 'v': {
				...
				kv_map[kv.substr(0, splitter)] = kv.substr(
				    splitter + 1);
			} break;
			default:
	...
	void ReportError()
	{
		if (flag_list) {
			...
			std::cout << "ident: __test_cases_list_" << kPytestName
				  << "_binary_"
				  << "not_found__" << std::endl;
		} else {
			std::cout << "execvp(" << kPytestName
